...
public class MultiBitValue {
  ...
  public MultiBitValue(final int length, final int position, final String toolTip) {
    if ((length <= 0) || (length > 32)) {
      throw new IllegalArgumentException(
          "Attempted to set length=" + length + ", which is larger than the maximum permitted value of 32 or is less than 1, in " + this);
    }
    if ((position < 0) || (position > 31)) {
      throw new IllegalArgumentException(
          "Attempted to set position=" + position + ", which is larger than the maximum permitted value of 31 or is negative, in " + this);
    }
  ...
  public int setValue(int configWord, int newValue) {
    if ((newValue < 0) || (newValue >= (1 << length))) {
      throw new IllegalArgumentException("Attempted to store value=" + newValue + ", which is larger than the maximum permitted value of " + ((1 << length) - 1)
          + " or is negative, in " + this);
    }
