...
public class CochleaAMS1bBasicPanel extends javax.swing.JPanel implements PropertyChangeListener {
  ...
  private void initComponents() {
    ...
    jLabel1.setText(
        "<html>This panel allows \"tweaking\" bias values around the nominal ones loaded from the XML file. Change made here are <b>not</b> permanent until the settings are saved to an XML file. On restart, these new settings will then become the nominal settings.");
    add(jLabel1);
  ...
  private void VgainTweakerStateChanged(javax.swing.event.ChangeEvent evt) {// GEN-FIRST:event_VgainTweakerStateChanged
    ...
    float ratio = (float) Math.exp(value * Math.log(Vgain_scale)); // Calculation to achieve the same changes as changeByRationFromPreferred; function
                                                                   // "changeByRationFromPreferred" not implemented in Pot class
  ...
  private void NeuronVLeakTweakerStateChanged(javax.swing.event.ChangeEvent evt) {// GEN-FIRST:event_NeuronVLeakTweakerStateChanged
    ...
    float ratio = (float) Math.exp(value * Math.log(NeuronVLeak_scale)); // Calculation to achieve the same changes as changeByRationFromPreferred; function
                                                                         // "changeByRationFromPreferred" not implemented in Pot class
  ...
  private void VqTweakerStateChanged(javax.swing.event.ChangeEvent evt) {// GEN-FIRST:event_VqTweakerStateChanged
    ...
    float ratio = (float) Math.exp(value * Math.log(Vq_scale)); // Calculation to achieve the same changes as changeByRationFromPreferred; function
                                                                // "changeByRationFromPreferred" not implemented in Pot class
