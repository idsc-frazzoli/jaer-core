...
public class CochleaTow4Ear extends CochleaChip implements Observer {
    ...
    for (final DisplayMethod m : getCanvas().getDisplayMethods()) {
      if ((m instanceof ChipRendererDisplayMethod) || (m instanceof SpaceTimeEventDisplayMethod)) {
        ...
        m.addAnnotator(new FrameAnnotater() {
          ...
          public void renderStrokeFontString(final GL2 gl, final float x, final float y, final float z, final float angleDeg, final String s) {
  ...
  public class Biasgen extends net.sf.jaer.biasgen.Biasgen implements net.sf.jaer.biasgen.ChipControlPanel {
    ...
    public Biasgen(final Chip chip) {
      ...
      ssBiases[0] = ssp;
      ssBiases[1] = ssn;
      SelResSW = new SPIConfigBit("SelResSW", "In diff1 cell, if SelResSW=1, gate of resistor is tied to SSP, otherwise to Vdd.", CypressFX3.FPGA_CHIPBIAS,
          (short) 156, false, this);
      ...
      // Scanner module
      scannerControl.add(new SPIConfigBit("ScannerEnable", "Enable scanner output.", CypressFX3.FPGA_SCANNER, (short) 0, false, this));
      scannerControl.add(new SPIConfigInt("ScannerChannel", "Which channel to scan out.", CypressFX3.FPGA_SCANNER, (short) 1, 7, 0, this));
      ...
      // Multiplexer module
      biasForceEnable = new SPIConfigBit("ForceBiasEnable", "Force the biases to be always ON.", CypressFX3.FPGA_MUX, (short) 3, false, this);
      biasForceEnable.addObserver(this);
      allPreferencesList.add(biasForceEnable);
      // Generic AER from chip
      aerControl.add(new SPIConfigBit("TestAEREnable", "Enable Test AER output instead of normal AER.", CypressFX3.FPGA_SCANNER, (short) 2, false, this)); // In
                                                                                                                                                           // scanner
                                                                                                                                                           // module
                                                                                                                                                           // for
                                                                                                                                                           // convenience.
      aerControl.add(new SPIConfigBit("AERRun", "Run the main AER state machine.", CypressFX3.FPGA_DVS, (short) 3, false, this));
      aerControl.add(new SPIConfigInt("AERAckDelay", "Delay AER ACK by this many cycles.", CypressFX3.FPGA_DVS, (short) 4, 12, 0, this));
      aerControl.add(new SPIConfigInt("AERAckExtension", "Extend AER ACK by this many cycles.", CypressFX3.FPGA_DVS, (short) 6, 12, 0, this));
      aerControl.add(new SPIConfigBit("AERWaitOnTransferStall",
          "Whether the AER state machine should wait,<br> or continue servicing the AER bus when the FIFOs are full.", CypressFX3.FPGA_DVS, (short) 8, false,
          this));
      aerControl.add(new SPIConfigBit("AERExternalAERControl", "Do not control/ACK the AER bus anymore, <br>but let it be done by an external device.",
      ...
      chipControl.add(new SPIConfigInt("AnalogMux0", "MSB First", CypressFX3.FPGA_CHIPBIAS, (short) 128, 4, 0, this));
      chipControl.add(new SPIConfigInt("PreampGain", "MSB First", CypressFX3.FPGA_CHIPBIAS, (short) 129, 2, 0, this));
      chipControl.add(new SPIConfigBit("Seln1", "Set to 0 for parallel architecture, Set to 1 for cascaded architecture, settings for LR pair",
          CypressFX3.FPGA_CHIPBIAS, (short) 130, false, this));
      chipControl.add(new SPIConfigBit("Seln2", "Set to 0 for parallel architecture, Set to 1 for cascaded architecture, settings for TB pair",
          CypressFX3.FPGA_CHIPBIAS, (short) 131, false, this));
      chipControl.add(new SPIConfigBit("SMconfigArow", "Used in On-chip Arbiter", CypressFX3.FPGA_CHIPBIAS, (short) 132, false, this));
      chipControl.add(new SPIConfigBit("DC5", "Not Used", CypressFX3.FPGA_CHIPBIAS, (short) 133, false, this));
      chipControl.add(
          new SPIConfigBit("SelBot", "Set this bit to 1 to disable Neuron Kill mechanism for neurons 0, 2 when AERKillBit is set to 1 and channel is selected",
              CypressFX3.FPGA_CHIPBIAS, (short) 134, true, this));
      chipControl.add(
          new SPIConfigBit("SelTop", "Set this bit to 1 to disable Neuron Kill mechanism for neurons 1, 3 when AERKillBit is set to 1 and channel is selected",
              CypressFX3.FPGA_CHIPBIAS, (short) 135, true, this));
      chipControl.add(new SPIConfigBit("DC8", "nB0 for Test SOS", CypressFX3.FPGA_CHIPBIAS, (short) 136, false, this));
      chipControl.add(new SPIConfigBit("DC9", "nB1 for Test SOS", CypressFX3.FPGA_CHIPBIAS, (short) 137, false, this));
      chipControl.add(new SPIConfigBit("DC10", "nB2 for Test SOS", CypressFX3.FPGA_CHIPBIAS, (short) 138, false, this));
      chipControl.add(new SPIConfigBit("DC11", "nB3 for Test SOS", CypressFX3.FPGA_CHIPBIAS, (short) 139, false, this));
      chipControl.add(new SPIConfigBit("DC12", "nB4 for Test SOS", CypressFX3.FPGA_CHIPBIAS, (short) 140, false, this));
      chipControl.add(new SPIConfigBit("DC13", "B5 for Test SOS", CypressFX3.FPGA_CHIPBIAS, (short) 141, false, this));
      chipControl.add(new SPIConfigBit("DC14", "B6 for Test SOS", CypressFX3.FPGA_CHIPBIAS, (short) 142, false, this));
      chipControl.add(new SPIConfigBit("DC15", "B7 for Test SOS", CypressFX3.FPGA_CHIPBIAS, (short) 143, false, this));
      chipControl.add(new SPIConfigBit("DC16", "B8 for Test Diff", CypressFX3.FPGA_CHIPBIAS, (short) 144, false, this));
      chipControl.add(new SPIConfigBit("DC17", "B9 for Test Diff", CypressFX3.FPGA_CHIPBIAS, (short) 145, false, this));
      chipControl.add(new SPIConfigBit("DC18", "B10 for Test Diff", CypressFX3.FPGA_CHIPBIAS, (short) 146, false, this));
      chipControl
          .add(new SPIConfigBit("BitTestLatch", "Set to 1 to enable followers of test signals to pad", CypressFX3.FPGA_CHIPBIAS, (short) 147, false, this));
      chipControl
          .add(new SPIConfigBit("DC20", "Bit set to 1 if more Leak Current is needed on Test Neuron", CypressFX3.FPGA_CHIPBIAS, (short) 148, false, this));
      chipControl.add(new SPIConfigBit("DC21", "Not Used", CypressFX3.FPGA_CHIPBIAS, (short) 149, false, this));
      chipControl.add(new SPIConfigBit("DC22", "Not Used", CypressFX3.FPGA_CHIPBIAS, (short) 150, false, this));
      chipControl.add(new SPIConfigBit("DC23", "Not Used", CypressFX3.FPGA_CHIPBIAS, (short) 151, false, this));
      chipControl.add(new SPIConfigInt("DigitalMux0", "?", CypressFX3.FPGA_CHIPBIAS, (short) 152, 4, 0, this));
      chipControl.add(new SPIConfigInt("DigitalMux1", "?", CypressFX3.FPGA_CHIPBIAS, (short) 153, 4, 0, this));
      chipControl.add(new SPIConfigInt("DigitalMux2", "?", CypressFX3.FPGA_CHIPBIAS, (short) 154, 4, 0, this));
      chipControl.add(new SPIConfigInt("DigitalMux3", "?", CypressFX3.FPGA_CHIPBIAS, (short) 155, 4, 0, this));
      chipControl
          .add(new SPIConfigBit("VresetBn", "If VresetBn_S = 1, all local latched AER KillBits in the neuron are cleared, that is neurons can send AER spikes",
              CypressFX3.FPGA_CHIPBIAS, (short) 157, false, this));
      chipControl.add(new SPIConfigInt("KillSignalDuration", "In 100MHz clock cycles", CypressFX3.FPGA_CHIPBIAS, (short) 164, 11, 2000, this));
      chipControl.add(new SPIConfigInt("KillChannel", "Select the channel to kill neurons in", CypressFX3.FPGA_CHIPBIAS, (short) 160, 8, 0, this));
