 ...
 * the ADCSamples object, resetting it in the process, and passed to the Series plotting object. In the meantime the USB thread is filling the other ADCSamples
 * buffer.
 * <P>
 * In the image case, the data in the ADCSamples object is not reset by the acquisition. Rather, whatever data is in the ADCSamples object is passed to Series
 * after the Series has been reset.
...
public class CochleaAMS1cRollingCochleagramADCDisplayMethod extends RollingCochleaGramDisplayMethod {
  ...
  public CochleaAMS1cRollingCochleagramADCDisplayMethod(ChipCanvas canvas) { // TODO fix all DisplayMethods so that they can accept the AEChip object and not
                                                                             // the canvas, or else fix here so that given canvas with null chip (as is the case
                                                                             // during AEChip creation) we can still do a lazy instantiation of necesary objects
    super(canvas);
