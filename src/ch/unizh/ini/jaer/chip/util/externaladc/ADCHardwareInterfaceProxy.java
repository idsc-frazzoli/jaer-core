...
public class ADCHardwareInterfaceProxy extends HardwareInterfaceProxy implements ADCHardwareInterface, HasPropertyTooltips {
  ...
  public ADCHardwareInterfaceProxy(Chip chip) {
    ...
    tooltipSupport.setPropertyTooltip("sequencingEnabled",
        "if enabled, then channels are sampled in sequence starting from channel 0 up to adcChannel. If cleared, then only the adcChannel is sampled.");
  }
  ...
  public void setADCChannel(int channel) {
    if (adcChannel < minADCchannel)
      adcChannel = minADCchannel;
    else if (adcChannel > maxADCchannel)
      adcChannel = maxADCchannel;
