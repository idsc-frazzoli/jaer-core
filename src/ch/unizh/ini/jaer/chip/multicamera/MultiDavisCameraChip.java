...
abstract public class MultiDavisCameraChip extends DavisBaseCamera implements MultiCameraInterface {
  ...
  final public class SelectCamera extends DavisMenuAction {
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      try {
        displaycamera = (int) JOptionPane.showInputDialog(frame, "Select camera to display:", "Choose Camera", JOptionPane.QUESTION_MESSAGE, null,
            possibilities, possibilities[NUM_CAMERAS]);
      } catch (Exception ex) {
  ...
  public HardwareInterface getHardwareInterface() {
    ...
    if (n < 1) {
      log.warning(
          " couldn't build MultiCameraHardwareInterface hardware interface because only " + n + " camera is available and at least 2 cameras are needed");
