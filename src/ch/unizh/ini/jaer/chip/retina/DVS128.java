...
public class DVS128 extends AETemporalConstastRetina implements Serializable, Observer, RemoteControlled {
  ...
  public static final String CMD_TWEAK_THESHOLD = "threshold", CMD_TWEAK_ONOFF_BALANCE = "balance", CMD_TWEAK_BANDWIDTH = "bandwidth",
      CMD_TWEAK_MAX_FIRING_RATE = "maxfiringrate";
  ...
  public DVS128() {
    ...
    if (getRemoteControl() != null) {
      ...
      getRemoteControl().addCommandListener(this, CMD_TWEAK_ONOFF_BALANCE,
          CMD_TWEAK_ONOFF_BALANCE + " val - tweaks on/off balance; increase for more ON events. val in range -1.0 to 1.0.");
      getRemoteControl().addCommandListener(this, CMD_TWEAK_MAX_FIRING_RATE,
          CMD_TWEAK_MAX_FIRING_RATE + " val - tweaks max firing rate; increase to reduce refractory period. val in range -1.0 to 1.0.");
      getRemoteControl().addCommandListener(this, CMD_TWEAK_THESHOLD,
          CMD_TWEAK_THESHOLD + " val - tweaks threshold; increase to raise threshold. val in range -1.0 to 1.0.");
    }
      ...
      if (arrayResetMenuItem == null) {
        ...
        arrayResetMenuItem.addActionListener(new ActionListener() {
          ...
          public void actionPerformed(ActionEvent evt) {
            ...
            if (!(hw instanceof HasResettablePixelArray)) {
              log.warning("cannot reset pixels with hardware interface=" + hw + " (class " + (hw != null ? hw.getClass() : null)
                  + "), interface doesn't implement HasResettablePixelArray");
              return;
            }
        ...
        setArrayResetMenuItem.addActionListener(new ActionListener() {
          ...
          public void actionPerformed(ActionEvent evt) {
            ...
            if (!(hw instanceof HasResettablePixelArray)) {
              log.warning(
                  "cannot reset pixels with hardware interface=" + hw + " (class " + hw.getClass() + "), interface doesn't implement HasResettablePixelArray");
              return;
            }
  ...
  public void onRegistration() {
    ...
    helpMenuItem2 = getAeViewer().addHelpURLItem(USER_GUIDE_URL_EDVS, "eDVS4337 user guide",
        "Opens user guide for eDVS4337 (128x128) silicon retina based on NXP LPC4337 microcontroller with FTDI serial USB and WLAN interfaces");
    helpMenuItem3 = getAeViewer().addHelpURLItem(FIRMWARE_CHANGELOG, "DVS128 Firmware Change Log",
        "Displays the head version of the DVS128 firmware change log");
      ...
      for (int i = 0; i < n; i += skipBy) { // TODO bug here?
        ...
        if ((addr & (CypressFX2DVS128HardwareInterface.SYNC_EVENT_BITMASK | BasicEvent.SPECIAL_EVENT_BIT_MASK)) != 0) { // msb is set
          ...
          if (printedSyncBitWarningCount > 0) {
            log.warning(
                "BasicEvent.address=" + e.address + " , raw address=" + addr + " is >32767 (0xefff); either sync (external input event) or stereo bit is set");
