...
public class DVSBiasController extends EventFilter2D implements FrameAnnotater {
  ...
  public DVSBiasController(AEChip chip) {
    ...
    setPropertyTooltip("rateHysteresis",
        "hysteresis for state change; after state entry, state exited only when avg rate changes by this factor from threshold");
    setPropertyTooltip("hysteresisFactor",
        "hysteresis for state change; after state entry, state exited only when avg rate changes by this factor from threshold");
  ...
  public void annotate(GLAutoDrawable drawable) {
    ...
    glut.glutBitmapString(GLUT.BITMAP_HELVETICA_12,
        String.format("Low passed event rate=%s Hz, state=%s", fmt.format(rateEstimator.getFilteredEventRate()), state.toString()));
    gl.glPopMatrix();
  }
