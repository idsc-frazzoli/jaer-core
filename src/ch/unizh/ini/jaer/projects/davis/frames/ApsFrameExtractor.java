  ...
  public void saveImage() {
    ...
    for (int y = 0; y < chip.getSizeY(); y++) {
      for (int x = 0; x < chip.getSizeX(); x++) {
        ...
        final int value = ((int) (256 * apsDisplay.getPixmapArray()[idx]) << 16) | ((int) (256 * apsDisplay.getPixmapArray()[idx + 1]) << 8)
            | (int) (256 * apsDisplay.getPixmapArray()[idx + 2]);
  ...
  private class MouseInfo extends MouseMotionAdapter {
    ...
    public void mouseMoved(final MouseEvent e) {
      ...
      if ((p.x >= 0) && (p.x < chip.getSizeX()) && (p.y >= 0) && (p.y < chip.getSizeY())) {
        ...
        EventFilter.log.info(String.format("reset= %d, signal= %d, reset-signal= %+d", (int) resetBuffer[idx], (int) signalBuffer[idx],
            (int) (resetBuffer[idx] - signalBuffer[idx])));
      }
    }
  }
}
