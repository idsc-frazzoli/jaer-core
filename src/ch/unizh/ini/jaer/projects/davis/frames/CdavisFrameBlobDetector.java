...
public class CdavisFrameBlobDetector extends EventFilter2D implements FrameAnnotater, PropertyChangeListener {
  ...
  public CdavisFrameBlobDetector(AEChip chip) {
    ...
    resetFilter();
    String General = "General", byColor = "byColor", byArea = "byArea", byCircularity = "byCircularity", byConvexity = "byConvexity",
        byInertiaRatio = "byInertiaRatio";
  ...
  synchronized public void propertyChange(PropertyChangeEvent evt) {
    ...
    if ((evt.getPropertyName() == AEFrameChipRenderer.EVENT_NEW_FRAME_AVAILBLE) && !chip.getAeViewer().isPaused() && colorBlobDetectionEnabled) {
  ...
  public void findColorBlobs(boolean drawAndSave) {
    ...
    SimpleBlobDetector blobDetector = SimpleBlobDetector
        .create(new SimpleBlobDetector.Params().filterByArea(filterByAreaEnabled).minArea(areaLowerBound).maxArea(areaUpperBound)
            .filterByColor(filterByColorEnabled).blobColor((byte) 255).filterByCircularity(filterByCircularityEnabled).minCircularity(circularityLowerBound)
            .maxCircularity(circularityUpperBound).filterByConvexity(filterByConvexityEnabled).minConvexity(convexityLowerBound)
            .maxConvexity(convexityUpperBound).filterByInertia(filterByInertiaEnabled).minInertiaRatio(inertiaLowerBound).maxInertiaRatio(inertiaUpperBound));
  ...
  public void trackBlobs(int frameTs) {
    if (colorBlobDetectionEnabled && (blobCenterVector != null)) {
      for (int i = 0; i < blobCenterVector.size(); i++) {
        ...
        if (size > 5) {
          RectangularClusterTracker.Cluster b = tracker
              .createCluster(new BasicEvent(frameTs, (short) blobCenter.pt().x(), (short) (480 - blobCenter.pt().y())));
