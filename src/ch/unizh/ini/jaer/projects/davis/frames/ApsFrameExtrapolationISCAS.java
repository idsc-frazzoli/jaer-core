 ...
 * C. Brandli, L. Muller, and T. Delbruck, “Real-Time, High-Speed Video Decompression Using a Frame- and Event-Based DAVIS Sensor,” in Proc. 2014 Intl. Symp.
...
public class ApsFrameExtrapolationISCAS extends EventFilter2D {
  ...
  public ApsFrameExtrapolationISCAS(AEChip chip) {
    super(chip);
    setPropertyTooltip("manualOnGain",
        "Gain for the rendering of the ON DVS event (if manual extrapolation method chosen by selecting freezeGains and revertLearning button)");
    setPropertyTooltip("manualOffGain",
        "Gain for the rendering of the ON DVS event (if manual extrapolation method chosen by selecting freezeGains and revertLearning button)");
    setPropertyTooltip("revertLearning", "Revert gains back to manual settings");
    setPropertyTooltip("clipping",
        "Clip the image to 0-1 range; use to reset reconstructed frame if it has diverged. Sometimes turning off clipping helps learn better gains, faster and more stably.");
    ...
    setPropertyTooltip("perPixelAndTimeBinErrorUpdateFactor",
        "Sets the rate that error affects gains of individual pixels always (independent of tieGainsToAverage) and per time bin since last event. Ideally 1 should correct the gains from a single frame. Range 0-1, typical value 0.4.");
    ...
    setPropertyTooltip("tieGainsToAverage",
        "All the gains are clamped to be identical to the average. Use this option to greatly speed up initial gain setting.");
    setPropertyTooltip("expansion",
        "Whether expansion of the extrapolation to neighboring pixels is used, to fill in missing pixels with missing events (not in published results)");
      ...
      for (int j = 0; j < numTimeBins; j++) {
        if (onTimeBlame[i][j] > 0) {
          if (Math.abs((pixelOffEventCounter[i] + pixelOnEventCounter[i])) > 0.01f) {
            onPixTimeBins[i][j] = onPixTimeBins[i][j]
                - perPixelAndTimeBinErrorUpdateFactor * pixelErrors[i] * onTimeBlame[i][j] / ((pixelOffEventCounter[i] + pixelOnEventCounter[i]));
          }
          newOnTimeBinSum[j] += onPixTimeBins[i][j];// onTimeBins[j] -
                                                    // binErrorMixingFactor*pixelErrors[i]*onTimeBlame[i][j]/((pixelOffEventCounter[i]+pixelOnEventCounter[i]));
          newOnTimeBinCount[j]++; // the commented part above is an alternative to the RHS
          onTimeBlame[i][j] = 0;
        }
        if (offTimeBlame[i][j] > 0) {
          if (Math.abs((pixelOffEventCounter[i] + pixelOnEventCounter[i])) > 0.01f) {
            offPixTimeBins[i][j] = offPixTimeBins[i][j]
                - perPixelAndTimeBinErrorUpdateFactor * pixelErrors[i] * offTimeBlame[i][j] / ((pixelOffEventCounter[i] + pixelOnEventCounter[i]));
          }
          newOffTimeBinSum[j] += offPixTimeBins[i][j];// offTimeBins[j] -
                                                      // binErrorMixingFactor*pixelErrors[i]*offTimeBlame[i][j]/((pixelOffEventCounter[i]+pixelOnEventCounter[i]));
