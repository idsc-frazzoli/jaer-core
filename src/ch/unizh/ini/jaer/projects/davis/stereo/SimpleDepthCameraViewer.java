...
class SimpleDepthCameraViewer extends Component implements VideoStream.NewFrameListener {
  float mHistogram[];
  int[] mImagePixels;
  ...
  public SimpleDepthCameraViewer() {
  }
  ...
  public synchronized void onFrameReady(VideoStream stream) {
    ...
    case SHIFT_9_3:
      ...
      while (frameData.remaining() > 0) {
        int depth = frameData.getShort() & 0xFFFF;
  ...
  private void calcHist(ByteBuffer depthBuffer) {
    ...
    if (points > 0) {
      for (int i = 1; i < mHistogram.length; i++) {
        mHistogram[i] = (int) (256 * (1.0f - (mHistogram[i] / points)));
      }
    }
  }

  public void saveFrame(String file) {
    File outputfile = new File(file);
    try {
      ImageIO.write(mBufferedImage, "jpg", outputfile);
    } catch (IOException e) {
      e.printStackTrace();
    }
  }
