...
public class SingleCameraCalibration extends EventFilter2D implements FrameAnnotater, Observer /* observes this to get informed about our size */ {
  ...
  public SingleCameraCalibration(AEChip chip) {
    ...
    setPropertyTooltip("undistortDVSevents",
        "applies LUT undistortion to DVS event address if calibration has been completed; events outside AEChip address space are filtered out");
  ...
  synchronized public EventPacket filterPacket(EventPacket in) {
    ...
    while (itr.hasNext()) {
      ...
      if (frameExtractor.hasNewFrame()) {
        ...
        if (patternFound && (captureTriggered || (autocaptureCalibrationFramesEnabled
            && System.currentTimeMillis() - lastAutocaptureTimeMs > autocaptureCalibrationFrameDelayMs && nAcqFrames < numAutoCaptureFrames))) {
          nAcqFrames++;
  ...
  public boolean findCurrentCorners(boolean drawAndSave) {
    ...
    if (drawAndSave) {
      ...
      if (locPatternFound) {
        ...
        // System.out.println(allImagePoints.get(n).getFloatBuffer().get(2 * i) + " " + allImagePoints.get(n).getFloatBuffer().get(2 * i + 1)+" |
        // "+allObjectPoints.get(n).getFloatBuffer().get(3 * i) + " " + allObjectPoints.get(n).getFloatBuffer().get(3 * i + 1) + " " +
        // allObjectPoints.get(n).getFloatBuffer().get(3 * i + 2));
        // }
        // }
      } else {
  ...
  public void annotate(GLAutoDrawable drawable) {
    ...
    if (!hideStatisticsAndStatus && (calibrationString != null)) {
      ...
      if (!textRendererScaleSet) {
        ...
        textRendererScale = TextRendererScale.draw3dScale(MultilineAnnotationTextRenderer.getRenderer(), longestString, chip.getCanvas().getScale(),
            chip.getSizeX(), .8f);
      }
    }
  }
  ...
  public void doCalibrate() {
    ...
    try {
      ...
      log.info("see http://docs.opencv.org/2.4/modules/calib3d/doc/camera_calibration_and_3d_reconstruction.html \n" + "\nCamera matrix: "
          ...
          + printMatD(distortionCoefs) + calibrationString);
    } catch (RuntimeException e) {
  ...
  private void generateCalibrationString() {
    ...
    sb.append(String.format("focal length avg=%.1f pixels=%.2f mm\nPrincipal point (green cross)=%.1f,%.1f, Chip size/2=%.0f,%.0f\n", focalLengthPixels,
        focalLengthMm, principlePoint.x, principlePoint.y, (float) chip.getSizeX() / 2, (float) chip.getSizeY() / 2));
