        ...
        if (filterOutput.isFromCochlea()) {
          ...
          if (panTiltFrame.panTiltControl != null) {
            if (panTiltFrame.panTiltControl.isWasMoving() == false) {
              if (panTiltFrame.panTiltControl.isConnected() && panTiltFrame.isUseCochlea()
                  && filterOutput.getConfidence() > panTiltFrame.getCochleaThreshold()) {
                if (panTiltFrame.isLearnRetina() && learnRetinaMoved) {
                  if (!panTiltFrame.isServoLimitTouched()) {
                    ...
                    // better would have been to move to: panTiltFrame.getPanPos() + filterOutput.getPanOffset() * panTiltFrame.getRetinaPanScaling() =
                    // learnRetinaLastPos + learnRetinaLastOffset * panTiltFrame.getRetinaPanScaling()
                    double betterScaling = (panTiltFrame.getPanPos() + filterOutput.getPanOffset() * panTiltFrame.getRetinaPanScaling() - learnRetinaLastPos)
                        / learnRetinaLastOffset;
                    double newScaling = (0.05 * betterScaling + 0.95 * panTiltFrame.getRetinaPanScaling());
                    log.info("Learning: scaling should be " + betterScaling + ". set RetinaPanScaling from " + panTiltFrame.getRetinaPanScaling() + " to "
                        + newScaling);
              ...
              if (panTiltFrame.panTiltControl.isConnected() && panTiltFrame.isUseRetina() && filterOutput.getConfidence() > panTiltFrame.getRetinaThreshold()) {
                if (panTiltFrame.isLearnCochlea() && learnCochleaMoved) {
                  if (!panTiltFrame.isServoLimitTouched()) {
                    ...
                    // better would have been to move to: panTiltFrame.getPanPos() + filterOutput.getPanOffset() * panTiltFrame.getRetinaPanScaling() =
                    // learnCochleaLastPos + learnCochleaLastOffset * panTiltFrame.getCochleaPanScaling()
                    double betterScaling = (panTiltFrame.getPanPos() + filterOutput.getPanOffset() * panTiltFrame.getRetinaPanScaling() - learnCochleaLastPos)
                        / learnCochleaLastOffset;
                    double newScaling = (0.05 * betterScaling + 0.95 * panTiltFrame.getCochleaPanScaling());
                    log.info("Learning: scaling should be " + betterScaling + ". set CochleaPanScaling from " + panTiltFrame.getCochleaPanScaling() + " to "
                        + newScaling);
