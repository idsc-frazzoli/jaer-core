...
public class ITDFilter extends EventFilter2D implements Observer, FrameAnnotater, RemoteControlled {
  ...
  public ITDFilter(AEChip chip) {
    ...
    setPropertyTooltip(udp, "sendITD_UDP_port",
        "hostname:port (e.g. localhost:9999) to send UDP ITD histograms to; messages are int32 seq # followed by int32 bin values");
    setPropertyTooltip(udp, "sendADCSamples",
        "true to send UDP ITD histograms to; messages are int32 seq # followed by int32 sample count followed by pairs of right/left short ADC samples totalling 2*count short sample values");
  ...
  public EventPacket<?> filterPacket(EventPacket<?> in) {
    ...
    for (Object e : in) {
      ...
      if (i.isSpecial() || i.isFilteredOut())
        continue;
      int ganglionCellThreshold;
      if (hasMultipleGanglionCellTypes
          && ((amsProcessingMethod == AMSprocessingMethod.NeuronsIndividually) || (amsProcessingMethod == AMSprocessingMethod.StoreSeparetlyCompareEvery))) {
      ...
      try {
        ...
        if (i.x >= numOfCochleaChannels) {
          log.warning("there was a BasicEvent i with i.x=" + i.x + " >= " + numOfCochleaChannels + "=numOfCochleaChannels! Therefore set numOfCochleaChannels="
              + (i.x + 1));
          ...
          do {
            ...
            if (absdiff < maxITD) {
              ...
              if (usePriorSpikeForWeight == true) {
                int weightTimeOtherSide = lastTs[i.x][ganglionCellThreshold][1 - ear][cursor]
                    - lastTs[i.x][ganglionCellThreshold][1 - ear][(cursor + 1) % dimLastTs];
    ...
    if (((now - lastUdpMsgTime) > MIN_UPD_PACKET_INTERVAL_MS) && sendITD_UDP_Messages && (client != null)) {
      ...
      try {
        ...
        // CochleaAMS1cADCSamples samples = coch.getAdcSamples(); // should have only new ADC samples obtained from this packet if we are running in the
        // AEViewer.ViewLoop.run thread
        // try {
        // samples.acquire();
        ...
        log.warning(boe.toString() + ": decrease number of histogram bins or increase rendering rate for less ADC samples per UDP packet to fit "
            + UDP_BUFFER_SIZE + " byte datagrams");
      }
    }
    ...
    if ((display == false) && (frame != null)) {
      ...
      if (frame == null) {
        try {
          ...
          // frame = null;
          // ITDFilter.this.display = false; // set this so we know that itdframe has been disposed so that next button press on doToggleITDDisplay works
          // correctly
          // }
          // });
