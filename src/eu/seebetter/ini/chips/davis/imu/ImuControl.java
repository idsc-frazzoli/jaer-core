...
public class ImuControl extends Observable implements HasPropertyTooltips, Biasgen.HasPreference, PreferenceChangeListener {
  ...
  public final void loadPreference() {
    try {
      setGyroScale(ImuGyroScale.valueOf(davisChip.getPrefs().get(getPreferencesKey() + "IMU.GyroScale", ImuGyroScale.GyroFullScaleDegPerSec1000.toString())));
      setAccelScale(ImuAccelScale.valueOf(davisChip.getPrefs().get(getPreferencesKey() + "IMU.AccelScale", ImuAccelScale.ImuAccelScaleG8.toString())));
  ...
  public void preferenceChange(final PreferenceChangeEvent e) {
    ...
    try {
      ...
      davisChip.getLog().warning(
          iae.toString() + ": Preference value=" + e.getNewValue() + " for the preferenc with key=" + e.getKey() + " is not a proper enum for an IMU setting");
    }
  }
}
