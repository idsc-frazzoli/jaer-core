...
abstract public class DavisBaseCamera extends DavisChip implements RemoteControlled {
  ...
  public void onRegistration() {
    ...
    helpMenuItem1 = getAeViewer().addHelpURLItem(DavisBaseCamera.HELP_URL_HW_USERGUIDES, "inilabs hardware user guides overview",
        "Opens product overview guide");
  ...
  public void setAutoshotThresholdEvents(int thresholdEvents) {
    if (thresholdEvents < 0) {
      ...
      WarningDialogWithDontShowPreference d = new WarningDialogWithDontShowPreference(null, false, "Long autoshot threshold",
          "<html>You have selected <i>Auto-Shot kevents/frame</i> mode, so that an APS frame is triggered every " + (thresholdEvents >> 10)
              + " <b>thousand<b> events. <p>Selecting a large" + " number here will make it appear that frame capture is not working");
      d.setVisible(true);
    }
  ...
  public class DavisEventExtractor extends RetinaExtractor {
        ...
        if ((incompleteIMUSampleException != null) || ((DavisChip.ADDRESS_TYPE_IMU & data) == DavisChip.ADDRESS_TYPE_IMU)) {
          ...
          default:
            if ((warningCount < 10) || ((warningCount % DavisEventExtractor.WARNING_COUNT_DIVIDER) == 0)) {
              Chip.log.warning("Event with unknown readout cycle was sent out! You might be reading a file that had the deprecated C readout mode enabled.");
            }
            warningCount++;
            break;
          }
     ...
     * @param timestamp
     * @return */
    protected ApsDvsEvent createApsFlagEvent(final OutputEventIterator outItr, final ApsDvsEvent.ReadoutType flag, final int timestamp) {
  ...
  public class DavisDisplayMethod extends ChipRendererDisplayMethodRGBA {
    ...
    public void display(final GLAutoDrawable drawable) {
      ...
      // System.out.println("[DHV] davisbasecam display");
    ...
    private void imuRender(final GLAutoDrawable drawable, final IMUSample imuSampleRender) {
      ...
      imuTextRenderer.draw3D(String.format("%.2f,%.2f dps", imuSampleRender.getGyroYawY(), imuSampleRender.getGyroTiltX()), x, y + 5, 0, textScale); // x,y,z,
                                                                                                                                                     // scale
                                                                                                                                                     // factor
      imuTextRenderer.end3DRendering();
      // gyro roll
      ...
      final String ratestr = String.format("IMU: last dt=%6.1fms temperature=%5.1fC", imuSampleRender.getDeltaTimeUs() * .001f,
    ...
    private void exposureRender(final GL2 gl) {
      ...
      final String s = String.format("Frame: %d; Exposure %.2f ms; Frame rate: %.2f Hz", getFrameCount(), getMeasuredExposureMs(), getFrameRateHz());
      ...
      for (int i = 0; i < nframes; i++) {
        gl.glRectf(nframes * rectw, getSizeY() + 1, ((nframes + 1) * rectw) - 3, (getSizeY() + (DavisDisplayMethod.FONTSIZE / 2)) - 1);
      }
      gl.glPopMatrix();
    }
  }
  ...
  final public class DecreaseExposureAction extends DavisMenuAction {
    public DecreaseExposureAction() {
      super("Decrease APS exposure", "<html>Decreases APS exposure<p>See <i>User-Friendly Controls</i> tab in HW configuration panel for more control",
          "DecreaseExposure");
  ...
  final public class IncreaseAPSExposure extends DavisMenuAction {
    public IncreaseAPSExposure() {
      super("Increase APS exposure", "<html>Increases APS exposure<p>See <i>User-Friendly Controls</i> tab in HW configuration panel for more control",
          "IncreaseExposure");
