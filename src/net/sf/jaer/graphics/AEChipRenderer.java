...
public class AEChipRenderer extends Chip2DRenderer implements PropertyChangeListener {
  ...
  public enum ColorMode {
    GrayLevel("Each event causes linear change in brightness"), Contrast(
        "Each event causes multiplicative change in brightness to produce logarithmic scale"), RedGreen("ON events are green; OFF events are red"), ColorTime(
            "Events are colored according to time within displayed slice, with red coding old events and green coding new events"), GrayTime(
                "Events are colored according to time within displayed slice, with white coding old events and black coding new events"), HotCode(
                    "Events counts are colored blue to red, blue=0, red=full scale");
  ...
  public synchronized void render(EventPacket packet) {
    System.out.println("[DHV] AECHIP RENDERER");
    ...
    setSpecialCount(0);
    try {
      // System.out.println(colorMode + " " + packet.getNumCellTypes());
      if (packet.getNumCellTypes() > 2) {
        ...
        case GrayLevel:
          if (!accumulateEnabled && !externalRenderer) {
            // resetFrame(0f); // also sets grayValue
  ...
  private Map<Integer, float[][]> typeColorsMap = new HashMap<>();
