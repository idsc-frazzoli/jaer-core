...
public class ImageDisplay extends GLJPanel implements GLEventListener {
  ...
  synchronized private void displayPixmap(GLAutoDrawable drawable) {
    ...
    // gl.glRasterPos2f(-.5f, -.5f); // to LL corner of chip, but must be inside viewport or else it is ignored, breaks on zoom if (zoom.isZoomEnabled() ==
    // false) {
  ...
  public void setEventPixmapNbd(int x, int y, int kernelextent, int chipsize, float[] array) {
    ...
    for (int i = -kernelextent; i < (kernelextent + 1); i++) {
      System.arraycopy(array, (3 * ((x - kernelextent) + ((y + i) * chipsize))), pixmap.array(), (3 * ((x - kernelextent) + ((y + i) * chipsize))),
          (3 * ((2 * kernelextent) + 1)));
    }
    pixmap.rewind();
  }
  ...
  public synchronized void reshape(GLAutoDrawable drawable, int x, int y, int width, int height) {
    ...
    // log.info("height=" + height + " width=" + width + " fillsHorizontally=" + fillsHorizontally + " fillsVertically=" + fillsVertically + " newscale=" +
    // newscale);
  ...
  private void drawText(GL2 gl) {
    ...
    if (xLabel != null) {
      ...
      textRenderer.draw(xLabel, (int) (getWidth() - r.getWidth()) / 2, (int) ((-clipArea.bottom / s) - ((xticks != null ? 2 : 1) * r.getHeight()))); // shift
                                                                                                                                                     // text
                                                                                                                                                     // down if
                                                                                                                                                     // there
                                                                                                                                                     // are tick
    ...
    if (yLabel != null) {
      ...
      gl.glTranslated((-r.getHeight() * s) - ((yticks != null ? 10 : 0) * s), (sizeY / 2) - ((r.getWidth() / 2) * s), 0); // shift label left if there are ytick
                                                                                                                          // markers
