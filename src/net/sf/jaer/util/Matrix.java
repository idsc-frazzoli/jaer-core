...
public strictfp class Matrix {
   ...
   * @param Y
   * @param X */
  public static void solve(final float A[][], final float Y[], float X[]) {
    int n = A.length;
    int m = n + 1;
   ...
   * @param V
   * @param Y */
  public static final void eigenCheck(final float A[][], final float V[][], float Y[]) {
    int n = A.length;
    float X[] = new float[n];
    float Z[] = new float[n];
    float T[] = new float[n];
  ...
  static void schur2(final float A[][], final int p, final int q, float c[], float s[]) {
    float tau;
    float t;
  ...
  static void mat22(final float c[], final float s[], final float A[][], float B[][]) {
  ...
  static void mat44(final int p, final int q, final float c[], final float s[], final float A[][], float V[][]) {
    int n = A.length;
  ...
  public static final void multiply(final float A[][], final float B[][], float C[][]) {
    ...
    if (B.length != nk || C.length != ni || C[0].length != nj) {
      log.warning("Error in Matrix.multiply, incompatible sizes in operation A*B=C: A.col(" + nk + ")==B.row(" + B.length + ") , C.row(" + C.length
          + ")==A.row(" + ni + ") , C.col(" + C[0].length + ")==B.col(" + nj + ")");
    }
    for (int i = 0; i < ni; i++)
      for (int j = 0; j < nj; j++) {
        C[i][j] = 0.0f;
  ...
  public final static float[][] addMatrix(float[][] a, float[][] b) {
    ...
    int cb = b[0].length;
    if (ca != cb || ra != rb) {
      log.warning("Matrix dimensions do not agree");
      return null;
    }
  ...
  public static final void copy(final float A[][], float B[][]) {
    ...
    if (B.length != ni || B[0].length != nj) {
      log.warning("Error in Matrix.copy," + " incompatible sizes.");
    }
    for (int i = 0; i < ni; i++)
      for (int j = 0; j < nj; j++)
        B[i][j] = A[i][j];
  ...
  public static final boolean equals(final float A[][], final float B[][]) {
    ...
    boolean same = true;
    if (B.length != ni || B[0].length != nj) {
      log.warning("Error in Matrix.equals," + " incompatible sizes.");
    }
    for (int i = 0; i < ni; i++)
      for (int j = 0; j < nj; j++)
