...
public class AbstractAviWriter extends EventFilter2DMouseAdaptor implements FrameAnnotater, PropertyChangeListener {
  ...
  public AbstractAviWriter(AEChip chip) {
    super(chip);
    setPropertyTooltip("startRecordingAndSaveAVIAs",
        "Opens the output file and starts writing to it. The AVI file is in RAW format with pixel values 0-255 coming from ApsFrameExtractor displayed frames, which are offset and scaled by it.");
    setPropertyTooltip("closeFile", "Closes the output file if it is open.");
    setPropertyTooltip("writeTimecodeFile", "writes a file alongside AVI file (with suffix " + TIMECODE_SUFFIX
        + ") that maps from AVI frame to AER timestamp for that frame (the frame end timestamp)");
    ...
    setPropertyTooltip("format",
        "video file is writtent to this output format (note that RLE will throw exception because OpenGL frames are not 4 or 8 bit images)");
    ...
    setPropertyTooltip("writeEnabled",
        "Selects if writing frames is enabled. Use this to temporarily disable output, or in conjunction with writeOnlyWhenMousePressed");
