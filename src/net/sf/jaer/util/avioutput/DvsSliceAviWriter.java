...
import net.sf.jaer.util.avioutput.Options.Multiplicity;
import net.sf.jaer.util.avioutput.Options.Separator;
...
public class DvsSliceAviWriter extends AbstractAviWriter implements FrameAnnotater {
  ...
  public DvsSliceAviWriter(AEChip chip) {
    ...
    setPropertyTooltip("writeDvsSliceImageOnApsFrame",
        "<html>write DVS slice image for each APS frame end event (dvsMinEvents ignored).<br>The frame is written at the end of frame APS event.<br><b>Warning: to capture all frames, ensure that playback time slices are slow enough that all frames are rendered</b>");
    setPropertyTooltip("normalizeFrame",
        "<html>Normalize the frame so that the 3-sigma range of original values fills the full output range of 0-1 values (0-255 in PNG file)<br>This normalization is the same as that used in DeepLearnCnnNetwork.");
    setPropertyTooltip("fullRectifyOutput",
        "<html>Fully rectify ON and OFF events to only ON events. <br>Output image ranges from 0-255 in PNG file but no events is 0 <br>and all events of either type produce positive pixel values");
  }
    ...
    for (BasicEvent e : in) {
      ...
      if ((writeDvsSliceImageOnApsFrame && newFrameAvailable && e.timestamp >= endOfFrameTimestamp)
          || (!writeDvsSliceImageOnApsFrame && dvsSubsampler.getAccumulatedEventCount() > dvsMinEvents)
              && (chip.getAeViewer() == null || !chip.getAeViewer().isPaused())) { // added check for nonnull aeviewer in case filter is called from separate
                                                                                   // program
  ...
  public void annotate(GLAutoDrawable drawable) {
    ...
    String s = null;
    if (normalizeFrame) {
      s = String.format("mostOffCount=%d\n mostOnCount=%d\navg frame rate=%.1f\nsparsity=%.2f%%", dvsSubsampler.getMostOffCount(),
          dvsSubsampler.getMostOnCount(), avgFrameRate, 100 * dvsSubsampler.getSparsity());
    } else {
  ...
  public synchronized void doStartRecordingAndSaveAVIAs() {
    String[] s = { "dimx=" + dimx, "dimy=" + dimy, "grayScale=" + grayScale, "dvsMinEvents=" + dvsMinEvents, "format=" + format.toString(),
  ...
  public static final String USAGE = "java DvsSliceAviWriter [-aechip=aechipclassname (fully qualified class name, e.g. eu.seebetter.ini.chips.davis.DAVIS240C)] "
      ...
      + "[-numevents=2000] [-rectify=false] [-normalize=true] [-showoutput=true]  [-maxframes=0] " + "inputFile.aedat [outputfile.avi]" + "\n"
  ...
  public static void main(String[] args) {
    ...
    while (writerControl.writing) {
      try {
        aeRaw = ais.readPacketByNumber(writer.getDvsMinEvents()); // read at most this many events to avoid writing duplicate frames at end of movie from start
                                                                  // of file, which would happen automatically by
    ...
    System.out.println(String.format(
        ...
        chipname, writer.getDimx(), writer.getDimy(), writer.getCompressionQuality(), writer.getFormat().toString(), writer.getFrameRate(),
        writer.getGrayScale(), writer.isWriteDvsSliceImageOnApsFrame(), writer.isWriteTimecodeFile(), writer.getDvsMinEvents(), writer.isFullRectifyOutput(),
        writer.isFullRectifyOutput(), writer.isShowOutput(), writer.getMaxFrames()));
