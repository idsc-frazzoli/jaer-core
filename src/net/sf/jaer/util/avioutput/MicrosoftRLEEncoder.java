...
public class MicrosoftRLEEncoder {
  ...
  private final DataChunkOutputStream temp = new DataChunkOutputStream(tempSeek);
  ...
  public void writeKey8(OutputStream out, byte[] data, int offset, int length, int step, int height) throws IOException {
    tempSeek.reset();
   ...
   * @param step
   * The number to add to offset to get to the next scanline. */
  public void writeDelta8(OutputStream out, byte[] data, byte[] prev, int offset, int length, int step, int height) throws IOException {
    tempSeek.reset();
