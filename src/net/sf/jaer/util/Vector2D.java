...
public class Vector2D extends Point2D.Float {
  ...
  public void unify() {
    if (length() == 0)
      throw new IllegalStateException("The length of the vector is 0, thus it can not be unified!");
    if (length() == 1)
      return; // already unitvector, no need to devide.
    div(length());
  }
  ...
  public double getAngle(Vector2D vec) {
    if (this.length() == 0)
      throw new IllegalStateException("The length of this vector is 0, the angle can not be computed!");
    if (vec.length() == 0)
      throw new IllegalStateException("The length of the argument vector is 0, the angle can not be computed!");
    double arg = this.dotProduct(vec) / (this.length() * vec.length());
    if (arg > 1)
      arg = 1; // can happen due to floating point error
    if (arg < -1)
      arg = -1;
