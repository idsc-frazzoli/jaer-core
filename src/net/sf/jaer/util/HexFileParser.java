...
public class HexFileParser {
  ...
  /** Constructs a CustomFileReader object.
   *
   * @param resourcePath intel hex file as resource. This filename is a full pathname giving the location of the file somewhere in the classpath (e.g. in the
   * jar of the project).
   * @exception FileNotFoundException If the file can't be found */
  public HexFileParser(File resourcePath) throws FileNotFoundException, IOException {
    ...
    br = new BufferedReader(new FileReader(resourcePath));
    if (br == null) {
      throw new IOException("can't open intel hex format hex Cypress FX2 firmware File " + resourcePath);// firmwareFileStream = new BufferedReader(new
                                                                                                         // FileReader(file));
    }
    records = new ArrayList<Record>();
    parseFile();
    ...
    while ((record = br.readLine()) != null) {
      ...
      for (j = 9, k = 0, l = address; j < temp; j = j + 2, k++, l++) {
        if (address > MAX_CODE_LENGTH) {
          throw new IllegalArgumentException("invalid address in record " + recordNum);
        }
      ...
      checksum = (byte) (Integer.parseInt(record.substring((recordLength - 2), recordLength), RADIX));
      if (DEBUG) {
