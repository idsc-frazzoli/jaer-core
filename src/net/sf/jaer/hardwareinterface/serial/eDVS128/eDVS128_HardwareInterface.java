          ...
          if (bCalibrated == 0) {
            ...
            if (length > 100) {
              for (int i = 0; i <= 3; i++) {
                ...
                diff = 0;
                for (int m = 0; m < 10; m++) {
                  diff += (buffer[(4 * (m + 1)) + i] - buffer[(4 * m) + i]) * (buffer[(4 * (m + 1)) + i] - buffer[(4 * m) + i]);
                }
    ...
    synchronized (aePacketRawPool) {
      ...
      while (i < bytesSent) {
        ...
        if ((y_ & 0x80) != 0x80) {
          ...
          addresses[eventCounter] = (((x_ & cHighBitMask) >> 7) | ((y_ & cLowerBitsMask) << 8) | ((x_ & cLowerBitsMask) << 1)) & 0x7FFF;
          ...
          if (lastshortts > shortts) { // timetamp wrapped
            ...
            if (DEBUG) {
              log.info("This timestamp was less than last one by " + (shortts - lastshortts) + " and system deltaT=" + ((thisWrapTime - lastWrapTime) / 1000)
                  + "us, wrapAdd=" + (wrapAdd / WRAP) + " wraps");
            }
