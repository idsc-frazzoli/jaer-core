 ...
 * @author tobi */
public class SiLabsC8051F320_USBIO_DVS128 extends UsbIoReader
    implements UsbIoErrorCodes, USBInterface, BiasgenHardwareInterface, AEMonitorInterface, ReaderBufferControl {
  @Override
  ...
  public void open() throws HardwareInterfaceException {
    ...
    status = bind(interfaceNumber, (byte) ENDPOINT_IN, gDevList, GUID); // binds this pipe (the reader) to the IN endpoint. We'll make another pipe to write
                                                                        // config.
    if (status != USBIO_ERR_SUCCESS) {
      UsbIo.destroyDeviceList(gDevList);
  ...
  public void startThread(int MaxIoErrorCount) {
    ...
    T.setPriority(MONITOR_PRIORITY); // very important that this thread have priority or the acquisition will stall on device side for substantial amounts of
                                     // time!
    T.setName("AEReader");
