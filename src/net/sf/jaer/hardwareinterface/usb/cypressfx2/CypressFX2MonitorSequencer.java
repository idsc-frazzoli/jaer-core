...
public class CypressFX2MonitorSequencer extends CypressFX2 implements AEMonitorSequencerInterface, AESoftMapper, HasUpdatableFirmware {
  ...
  // static final byte VR_WRITE_EEPROM_BYTES =(byte)0xC8; // write the first 8 EEPROM bytes (VID,PID,DID), same functionality exists in CypressFX2, has to be
  // cleaned up
  ...
  public class AEWriter extends UsbIoWriter {
    ...
    public void startThread(int MaxIoErrorCount) {
      ...
      try {
        T.setPriority(Thread.MAX_PRIORITY - 2); // very important that this thread have priority or the acquisition will stall on device side for substantial
                                                // amounts of time!
      } catch (Exception e) {
