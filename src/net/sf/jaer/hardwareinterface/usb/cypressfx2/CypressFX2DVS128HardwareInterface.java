 ...
 *
 * @author tobi/rapha */
public class CypressFX2DVS128HardwareInterface extends CypressFX2Biasgen
    implements HasUpdatableFirmware, HasResettablePixelArray, HasSyncEventOutput, HasLEDControl {
  ...
  private boolean syncEventEnabled = true; // tobi changed to always true so that if user selects this mode, then it must be deselected explicitly for slave
                                           // cameras // default is true so that device is the timestamp master by default, necessary after firmware rev 11
  ...
  synchronized public void resetTimestamps() {
    log.info(
        this + ".resetTimestamps(): zeroing timestamps by sending vendor request to hardware which should return timestamp-reset event and reset wrap counter");
    try {
  ...
  public class RetinaAEReader extends CypressFX2.AEReader {
    ...
    protected void translateEvents(UsbIoBuf b) {
      ...
      synchronized (aePacketRawPool) {
        ...
        // if(buffer.overrunOccuredFlag) return; // don't bother if there's already an overrun, consumer must get the events to clear this flag before there is
        // more room for new events
        int shortts;
        ...
        for (int i = 0; i < bytesSent; i += 4) {
          ...
          if ((aeBuffer[i + 3] & 0x80) == 0x80) { // timestamp bit 15 is one -> wrap
            ...
            if ((resetTimestampWarningCount < RESET_TIMESTAMPS_INITIAL_PRINTING_LIMIT)
              ...
              log.info(this + ".translateEvents got reset event from hardware, timestamp "
                  + (0xffff & ((aeBuffer[i + 2] & 0xff) | ((aeBuffer[i + 3] & 0x3f) << 8))));
            }
            if (resetTimestampWarningCount == RESET_TIMESTAMPS_INITIAL_PRINTING_LIMIT) {
              log.warning("will only print reset timestamps message every " + RESET_TIMESTAMPS_WARNING_INTERVAL
    ...
    Thread T = new Thread("FirmwareUpdater") {
      ...
      public void run() {
        try {
          ...
          JOptionPane.showMessageDialog(chip.getAeViewer(), "Update successful - unplug and replug the device to activate new firmware",
