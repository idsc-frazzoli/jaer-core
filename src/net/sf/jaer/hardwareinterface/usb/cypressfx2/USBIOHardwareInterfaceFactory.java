...
public class USBIOHardwareInterfaceFactory implements UsbIoErrorCodes, PnPNotifyInterface, HardwareInterfaceFactoryInterface {
  ...
  synchronized void buildUsbIoList() {
    ...
    for (int i = 0; i < MAXDEVS; i++) {
      ...
      if (status == USBIO_ERR_NO_SUCH_DEVICE_INSTANCE) {
        ...
        if (status2 == USBIO_ERR_SUCCESS) { // only add device if it can be exclusively bound. If devices are alredy opened exclusively, then they will not
                                            // appear in the list
          usbioList.add(dev);
  ...
  synchronized public USBInterface getInterface(int n) {
    ...
    if (n > (numAvailable - 1)) {
      if (numAvailable == 0) {
        log.warning("You asked for interface number " + n
            + " but no interfaces are available. Check the Windows Device Manager to see if the device has been recognized. You may need to install a driver.");
      } else {
