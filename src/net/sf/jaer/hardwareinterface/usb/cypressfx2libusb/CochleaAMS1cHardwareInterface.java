...
public class CochleaAMS1cHardwareInterface extends CypressFX2Biasgen implements BiasgenHardwareInterface {
  ...
  public static final int DATA_CODE_MASK = 0xc000, DATA_CODE_DATA = 0x0000, DATA_CODE_TIMESTAMP = 0x4000, DATA_CODE_WRAP = 0x8000,
      DATA_CODE_TIMESTAMP_RESET = 0xd000;
  /** Data word is tagged as AER address or ADC sample by bit 0x2000 (DATA_TYPE_MASK). Set (ADDRESS_TYPE_ADC)=ADC sample; unset (DATA_TYPE_AER_ADDRESS)=AER
   * address. */
  ...
  public final byte VR_IS_TS_MASTER = (byte) 0xCB; // this VR make the board timestamp master if argument is 1, otherwise it is slave device (default in
                                                   // firmware is master device)
  ...
  public void setInEndpointEnabled(boolean inEndpointEnabled) throws HardwareInterfaceException {
    ...
    // still not clear why the configuration must be sent again after starting event acquisition, because nothing in firmware should reset the CPLD after the
    // configuration has already been sent
  }
            ...
            case 0: // data, either AER address or ADC sample
              ...
              if ((eventCounter >= aeBufferSize) || (buffer.overrunOccuredFlag)) {
                ...
                if (isADCSample(dataword)) {
                  ...
                  eventCounter++;
                  // System.out.println("ADC word: " + dataword + " adcChannel=" + adcChannel(dataword) + " adcSample=" + adcSample(dataword) + "
                  // isScannerSyncBit=" + isScannerSyncBit(dataword));
                } else { // received an address, write out event to addresses/timestamps output arrays, masking out other bits
                  addresses[eventCounter] = (dataword & AER_DATA_MASK); // ok to mask out other bits now for this AER address, since ADC samples preserve the
                                                                        // ADC ID bit
