 ...
 *
 * @author tobi/rapha */
public class CypressFX2DVS128HardwareInterface extends CypressFX2Biasgen implements HasResettablePixelArray, HasSyncEventOutput, HasLEDControl {
  ...
  private boolean syncEventEnabled = CypressFX2DVS128HardwareInterface.prefs.getBoolean("CypressFX2DVS128HardwareInterface.syncEventEnabled", true); // default
  // is
  // true
  // so
  // that
  ...
  public void setSyncEventEnabled(final boolean yes) {
    ...
    try {
      ...
      CypressFX2DVS128HardwareInterface.prefs.putBoolean("CypressFX2DVS128HardwareInterface.syncEventEnabled", yes);
    } catch (final HardwareInterfaceException e) {
      CypressFX2.log.warning(e.toString());
    }
  }
  ...
  public class RetinaAEReader extends CypressFX2.AEReader {
    ...
    protected void translateEvents(final ByteBuffer b) {
      synchronized (aePacketRawPool) {
        ...
        if ((bytesSent % 4) != 0) {
          CypressFX2.log.warning("CypressFX2.AEReader.translateEvents(): warning: " + bytesSent + " bytes sent, which is not multiple of 4");
        ...
        for (int i = 0; i < bytesSent; i += 4) {
          ...
          if ((b.get(i + 3) & 0x80) == 0x80) { // timestamp bit 15 is one -> wrap
            ...
            if ((resetTimestampWarningCount < RESET_TIMESTAMPS_INITIAL_PRINTING_LIMIT)
              ...
              CypressFX2.log.info(
                  this + ".translateEvents got reset event from hardware, timestamp " + (0xffff & ((b.get(i + 2) & 0xff) | ((b.get(i + 3) & 0x3f) << 8))));
            }
            if (resetTimestampWarningCount == RESET_TIMESTAMPS_INITIAL_PRINTING_LIMIT) {
              CypressFX2.log.warning("will only print reset timestamps message every " + RESET_TIMESTAMPS_WARNING_INTERVAL
            ...
            if (timestamps[eventCounter] < lastTimestampTmp) {
              CypressFX2.log.info("nonmonotonic timestamp: lastTimestamp=" + lastTimestampTmp + " timestamp=" + timestamps[eventCounter]);
            }
