...
public class LibUsbHardwareInterfaceFactory implements HardwareInterfaceFactoryInterface {
  ...
  private LibUsbHardwareInterfaceFactory() {
    ...
    try {
      ...
      UnsatisfiedLinkError u = new UnsatisfiedLinkError(
          "Failed to initialize libusb4java! Do you have noexec on your /tmp ? See jAERViewer1.5_linux.sh for a workaround.\n" + ule.getLocalizedMessage());
  ...
  synchronized public USBInterface getInterface(final int n) {
    ...
    if (n > (numAvailable - 1)) {
      if (numAvailable == 0) {
        LibUsbHardwareInterfaceFactory.log.warning(String.format("You asked for interface number %d but no interfaces are available at all. Check your Device "
            + "Manager to see if the device has been recognized. You may need to install a driver.", n));
      } else {
        LibUsbHardwareInterfaceFactory.log.warning(String.format("Only %d interfaces are available, but you asked for number %d (0 based).", numAvailable, n));
      }
      return null;
    }
    ...
    } catch (InstantiationException | IllegalAccessException | IllegalArgumentException | InvocationTargetException | NullPointerException e) {
      e.printStackTrace();
      return null;
    }
