...
public class CypressFX2 implements AEMonitorInterface, ReaderBufferControl, USBInterface, HasUsbStatistics {
  ...
  private class AsyncStatusThread {
    ...
    public void startThread() {
      ...
      usbTransfer = new USBTransferThread(monitor.deviceHandle, CypressFX2.STATUS_ENDPOINT_ADDRESS, LibUsb.TRANSFER_TYPE_BULK, new ProcessStatusMessages(), 2,
          128);
      ...
      public void processTransfer(final RestrictedTransfer transfer) {
        if (transfer.status() != LibUsb.TRANSFER_COMPLETED) {
          if (transfer.status() != LibUsb.TRANSFER_CANCELLED) {
            CypressFX2.log.warning("Error waiting for completion of read on status pipe: " + LibUsb.errorName(transfer.status()));
          }
          return;
        }
        if (transfer.actualLength() > 0) {
          ...
          case 0x01: {
            ...
            if (reader != null) {
              ...
              CypressFX2.log.info("Received timestamp external reset message, but monitor is not running");
            }
            break;
          }
          case (byte) 0xFF:
            ...
            SwingUtilities.invokeLater(new Runnable() {
              @Override
              public void run() {
                support.firePropertyChange(CypressFX2.PROPERTY_CHANGE_ASYNC_STATUS_MSG, null, newBuf);
              }
            });
            break;
          default:
  ...
  public class AEReader implements ReaderBufferControl {
    ...
    public void startThread() {
      ...
      usbTransfer = new USBTransferThread(monitor.deviceHandle, (byte) 0x86, LibUsb.TRANSFER_TYPE_BULK, new ProcessAEData(), getNumBuffers(), getFifoSize(),
      ...
      public void processTransfer(final RestrictedTransfer transfer) {
        ...
        synchronized (aePacketRawPool) {
          if ((transfer.status() == LibUsb.TRANSFER_COMPLETED) || (transfer.status() == LibUsb.TRANSFER_CANCELLED)) {
            ...
            CypressFX2.log.warning("ProcessAEData: Bytes transferred: " + transfer.actualLength() + "  Status: " + LibUsb.errorName(transfer.status()));
          }
          if (timestampsReset) {
    ...
    public void setFifoSize(int fifoSize) {
      if (fifoSize < AEReader.CYPRESS_FIFO_SIZE) {
        CypressFX2.log.warning("CypressFX2 fifo size clipped to device FIFO size " + AEReader.CYPRESS_FIFO_SIZE);
  ...
  public void setAEBufferSize(final int size) {
    if ((size < 1000) || (size > 1000000)) {
      CypressFX2.log.warning("ignoring unreasonable aeBufferSize of " + size + ", choose a more reasonable size between 1000 and 1000000");
      return;
    }
    aeBufferSize = size;
  ...
  synchronized public void open() throws HardwareInterfaceException {
    ...
    if (LibUsb.getDeviceSpeed(device) != LibUsb.SPEED_HIGH) {
      CypressFX2.log.warning("Device is not operating at USB 2.0 High Speed, performance will be limited to about 300 keps");
    }
  ...
  protected boolean isBlankDevice() {
    if ((deviceDescriptor.idVendor() == CypressFX2.VID_BLANK) && (deviceDescriptor.idProduct() == CypressFX2.PID_BLANK)) {
