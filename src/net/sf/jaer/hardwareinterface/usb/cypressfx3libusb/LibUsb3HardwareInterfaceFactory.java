...
public class LibUsb3HardwareInterfaceFactory implements HardwareInterfaceFactoryInterface {
  ...
  synchronized public USBInterface getInterface(final int n) {
    ...
    if (n > (numAvailable - 1)) {
      if (numAvailable == 0) {
        LibUsb3HardwareInterfaceFactory.log.warning(String.format("You asked for interface number %d but no interfaces are available at all. Check your Device "
            + "Manager to see if the device has been recognized. You may need to install a driver.", n));
      } else {
        LibUsb3HardwareInterfaceFactory.log.warning(String.format("Only %d interfaces are available, but you asked for number %d (0 based).", numAvailable, n));
      }
      return null;
    }
