  ...
  public class RetinaAEReader extends CypressFX3.AEReader implements PropertyChangeListener {
    ...
    public RetinaAEReader(final CypressFX3 cypress) throws HardwareInterfaceException {
      ...
      if (getPID() == PID_FX2) {
        ...
        checkFirmwareLogic(DAViSFX3HardwareInterface.REQUIRED_FIRMWARE_VERSION_FX2, DAViSFX3HardwareInterface.REQUIRED_LOGIC_REVISION_FX2);
      } else {
        checkFirmwareLogic(DAViSFX3HardwareInterface.REQUIRED_FIRMWARE_VERSION_FX3, DAViSFX3HardwareInterface.REQUIRED_LOGIC_REVISION_FX3);
      }
    ...
    private void checkMonotonicTimestamp() {
      if (currentTimestamp <= lastTimestamp) {
        CypressFX3.log.severe(toString() + ": non strictly-monotonic timestamp detected: lastTimestamp=" + lastTimestamp + ", currentTimestamp="
            + currentTimestamp + ", difference=" + (lastTimestamp - currentTimestamp) + ".");
      }
    }
          ...
          if ((event & 0x8000) != 0) {
            ...
            case 0: // Special event
              ...
              case 7: // IMU End
                ...
                if (imuCount == ((2 * RetinaAEReader.IMU_DATA_LENGTH) + 1)) {
                  ...
                  CypressFX3.log.info("IMU End: failed to validate IMU sample count (" + imuCount + "), discarding samples.");
                }
                break;
              ...
              case 13: // APS Column End
                ...
                if (apsCountY[apsCurrentReadoutType] != apsROISizeY[0]) {
                  CypressFX3.log.severe("APS Column End: wrong row count [" + apsCurrentReadoutType + " - " + apsCountY[apsCurrentReadoutType]
                      + "] detected. You might want to enable 'Ensure APS data transfer' under 'HW Configuration -> Chip Configuration' to improve this.");
                }
            ...
            case 3: // X address, Polarity ON
              ...
              if (ensureCapacity(buffer, eventCounter + 1)) {
                ...
                final byte polarity = ((chipID == DAViSFX3HardwareInterface.CHIP_DAVIS208) && (data < 192)) ? ((byte) (~code)) : (code);
                if (dvsInvertXY) {
                  buffer.getAddresses()[eventCounter] = ((data << DavisChip.YSHIFT) & DavisChip.YMASK)
                      | (((dvsSizeY - 1 - dvsLastY) << DavisChip.XSHIFT) & DavisChip.XMASK) | (((polarity & 0x01) << DavisChip.POLSHIFT) & DavisChip.POLMASK);
                } else {
                  buffer.getAddresses()[eventCounter] = ((dvsLastY << DavisChip.YSHIFT) & DavisChip.YMASK)
                      | (((dvsSizeX - 1 - data) << DavisChip.XSHIFT) & DavisChip.XMASK) | (((polarity & 0x01) << DavisChip.POLSHIFT) & DavisChip.POLMASK);
                }
                buffer.getTimestamps()[eventCounter++] = currentTimestamp;
              }
              dvsGotY = false;
            ...
            case 4: // APS ADC sample
              ...
              // Check that the buffer has space for this event. Enlarge if needed.
              if (ensureCapacity(buffer, eventCounter + 1)) {
                buffer.getAddresses()[eventCounter] = DavisChip.ADDRESS_TYPE_APS | ((yPos << DavisChip.YSHIFT) & DavisChip.YMASK)
                    | ((xPos << DavisChip.XSHIFT) & DavisChip.XMASK) | ((apsCurrentReadoutType << DavisChip.ADC_READCYCLE_SHIFT) & DavisChip.ADC_READCYCLE_MASK)
            ...
            case 7: // Timestamp wrap
              ...
              // Check monotonicity of timestamps.
              checkMonotonicTimestamp();
              CypressFX3.log.fine(String.format("Timestamp wrap event received on %s with multiplier of %d.", super.toString(), data));
              break;
            default:
