...
public class CypressFX3 implements AEMonitorInterface, ReaderBufferControl, USBInterface, HasUsbStatistics {
  ...
  protected void checkFirmwareLogic(final int requiredFirmwareVersion, final int requiredLogicRevision) throws HardwareInterfaceException {
    ...
    if (usbFWVersion < requiredFirmwareVersion) {
      updateStringBuilder.append(
          String.format("Device firmware version too old. You have version %d; but at least version %d is required.\n", usbFWVersion, requiredFirmwareVersion));
    }
    final int logicRevision = spiConfigReceive(CypressFX3.FPGA_SYSINFO, (short) 0);
    if (logicRevision < requiredLogicRevision) {
      updateStringBuilder.append(
          String.format("Device logic revision too old. You have revision %d; but at least revision %d is required.\n", logicRevision, requiredLogicRevision));
    }
  ...
  public class AEReader implements ReaderBufferControl {
    ...
    public void startThread() {
      ...
      usbTransfer = new USBTransferThread(monitor.deviceHandle, CypressFX3.AE_MONITOR_ENDPOINT_ADDRESS, LibUsb.TRANSFER_TYPE_BULK, new ProcessAEData(),
          getNumBuffers(), getFifoSize(), null, null, new Runnable() {
            @Override
            public void run() {
      ...
      public void processTransfer(final RestrictedTransfer transfer) {
        synchronized (aePacketRawPool) {
          if (transfer.status() == LibUsb.TRANSFER_COMPLETED) {
            ...
            CypressFX3.log.warning("ProcessAEData: Bytes transferred: " + transfer.actualLength() + "  Status: " + LibUsb.errorName(transfer.status()));
          }
        }
      }
    }
  ...
  public void setAEBufferSize(final int size) {
    if ((size < 1000) || (size > 2000000)) {
      CypressFX3.log.warning("ignoring unreasonable aeBufferSize of " + size + ", choose a more reasonable size between 1000 and 1000000");
      return;
    }
    aeBufferSize = size;
  ...
  synchronized public void open() throws HardwareInterfaceException {
    ...
    if (isBlankDevice()) { // TODO throws null pointer exception if deviceDescriptor is not obtained above, which
      ...
      if (!success) {
        throw new HardwareInterfaceException("open(): couldn't reopen device after firmware download and re-enumeration: " + LibUsb.errorName(status));
      } else {
        throw new HardwareInterfaceException(
    ...
    if (deviceDescriptor.bNumConfigurations() != 1) {
      throw new HardwareInterfaceException("number of configurations=" + deviceDescriptor.bNumConfigurations() + " is not 1 like it should be");
    }
  ...
  synchronized public void sendVendorRequest(final byte request, final short value, final short index, final byte[] bytes) throws HardwareInterfaceException {
  ...
  synchronized public void sendVendorRequest(final byte request, final short value, final short index, final byte[] bytes, final int pos, final int length)
      throws HardwareInterfaceException {
  ...
  synchronized public void sendVendorRequest(final byte request, final short value, final short index, ByteBuffer dataBuffer)
    ...
    if (status < LibUsb.SUCCESS) {
      throw new HardwareInterfaceException("Unable to send vendor OUT request " + String.format("0x%x", request) + ": " + LibUsb.errorName(status));
    }
    if (status != dataBuffer.capacity()) {
      throw new HardwareInterfaceException("Wrong number of bytes transferred, wanted: " + dataBuffer.capacity() + ", got: " + status);
    }
  }
  ...
  synchronized public ByteBuffer sendVendorRequestIN(final byte request, final short value, final short index, final int dataLength)
    ...
    final int status = LibUsb.controlTransfer(deviceHandle, bmRequestType, request, value, index, dataBuffer, 0);
    if (status < LibUsb.SUCCESS) {
      throw new HardwareInterfaceException("Unable to send vendor IN request " + String.format("0x%x", request) + ": " + LibUsb.errorName(status));
    }
