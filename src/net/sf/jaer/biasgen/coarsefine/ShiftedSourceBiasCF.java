...
public class ShiftedSourceBiasCF extends AddressedIPot {
  ...
  public ShiftedSourceBiasCF(Biasgen biasgen, String name, int address, Type type, Sex sex, boolean lowCurrentModeEnabled, boolean enabled, int refBitValue,
      int regBitValue, int displayPosition, String tooltipString) {
    this(biasgen);
  ...
  public void setName(String name) {
    ...
    if (chip.getRemoteControl() != null) {
      chip.getRemoteControl().addCommandListener(this, String.format(SETREGBITVAL + "%s <bitvalue>", getName()),
          "Set the bufferBitValue of shifted source " + getName());
      chip.getRemoteControl().addCommandListener(this, String.format(SETREFBITVAL + "%s <bitvalue>", getName()),
          "Set the voltage bit level of shifted source " + getName());
      chip.getRemoteControl().addCommandListener(this, String.format(SETVLEVEL + "%s " + getEnumOptions(VoltageLevel.class), getName()),
          "Set the type of shifted source " + getName());
      chip.getRemoteControl().addCommandListener(this, String.format(SETMODE + "%s " + getEnumOptions(OperatingMode.class), getName()),
          "Set the current level of shifted source " + getName());
    }
  }
  ...
  public int computeBinaryRepresentation() {
    int ret = ((OperatingMode.mask & getOperatingMode().bits()) // 0x0003
        ...
        | ((refBiasMask & (bitValue << Integer.numberOfTrailingZeros(refBiasMask))) & 0xffff)); // 0x00fc
  ...
  static String KEY_REFVALUE = "RefValue", KEY_REGVALUE = "RegBitValue", KEY_VOLTAGELEVEL = "VoltageLevel", KEY_OPERATINGMODE = "OperatingMode";
  static String SEP = ".";

  @Override
  public void preferenceChange(PreferenceChangeEvent e) {
    // TODO we get pref change events here but by this time the new values have already been set and there is no change in value so the GUI elements are not
    // updated
    try {
  ...
  public String toString() {
    return "ShiftedSourceBias name=" + name + " Sex=" + getSex() + " bitValue=" + bitValue + " regBitValue=" + regBitValue + " refBitValue=" + refBitValue
        + " operatingMode=" + getOperatingMode() + " voltageLevel=" + getVoltageLevel() + " maxRefValue=" + maxRefBitValue + " maxRegBitValue="
        + maxRegBitValue;
  }
