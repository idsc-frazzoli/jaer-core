...
public class Biasgen implements BiasgenPreferences, Observer, BiasgenHardwareInterface {
  /** max number of bytes used for each bias. For 24-bit biasgen, only 3 bytes are used and the newest configurable bias generator uses only 2 bytes, but we
   * oversize considerably for the future. */
    ...
    for (int bite = 0; bite < nbytes; bite++) {
      ...
      for (int i = 0; i < 8; i++) {
        ...
        if (bitString.charAt(bit) == '1') {
          ...
          // this conditional and loop structure ensures we count in bytes and that we left shift for each bit position in the byte, padding on the right with
          // 0's
  ...
  /** Marks a class (for example, some object in a subclass of Biasgen) as having a preference that can be loaded and stored. Classes do *not* store preferences
   * unless
