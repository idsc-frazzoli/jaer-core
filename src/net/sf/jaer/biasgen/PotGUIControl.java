...
public class PotGUIControl extends javax.swing.JPanel implements Observer, StateEditable {
  ...
  protected void updateAppearance() {
    if (pot == null)
      return;
    if (typeLabel.isVisible() != typeEnabled) {
      typeLabel.setVisible(typeEnabled);
      rr();
    }
    if (sexLabel.isVisible() != sexEnabled) {
      sexLabel.setVisible(sexEnabled);
      rr();
    }
    if (bitValueTextField.isVisible() != bitValueEnabled) {
      bitValueTextField.setVisible(bitValueEnabled);
      rr();
    }
