 ...
 * <p>
 * It is necessary to reset the PotTweaker when new biases values are loaded from preferences or saved. This is done by adding the PotTweaker as a property
 * change listener to the Biasgen object, which generates PropertyChangeEvents when these actions occur. The following code snippet shows the code in the
 * constructor of the hardware configuration control object (Biasgen).
 * <pre>
 * 
...
public class PotTweaker extends javax.swing.JPanel implements Observer, ChangeListener, StateEditable, PropertyChangeListener {
  ...
  // if (!evt.getKey().endsWith("bgTabbedPaneSelectedIndex")) { // TODO have some way to detect when new preferences are loaded, not resepond to **all**
  // preference changes
