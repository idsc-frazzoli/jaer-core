...
public abstract class MultiSourceProcessor extends EventFilter2D {
  ...
  EventPacket mergePackets(ArrayList<EventPacket> packets) {
    ...
    while (!pq.isEmpty()) {
      ...
      if (buffers.get(ev.source).isEmpty()) { // If buffer did get input this round, return
        if (queueAlive[ev.source]) { // Mark the queue as dead, do not replace item polled from Priority Queue, return
