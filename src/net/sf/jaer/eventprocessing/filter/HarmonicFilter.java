...
public class HarmonicFilter extends EventFilter2D implements Observer, FrameAnnotater {
  ...
  private float threshold = getPrefs().getFloat("HarmonicFilter.threshold", 0.1f); // when value is less than this value, then we are crossing zero and don't
                                                                                   // pass events
  ...
  public HarmonicFilter(AEChip chip) {
    ...
    setPropertyTooltip("threshold",
        "increase to reduce # of events; when oscillator instantaneous value is within this value as fraction of amplitude, then events are blocked ");
