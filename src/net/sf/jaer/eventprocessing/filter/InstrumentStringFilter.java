 ...
 */
//import ch.unizh.ini.jaer.projects..PixelBuffer;
...
public class InstrumentStringFilter extends EventFilter2D implements Observer, FrameAnnotater {
  ...
  private float halfWidthOfAveragingWindow = getPrefs().getFloat("InstrumentStringFilter.halfWidthOfAveragingWindow (in tones)", (float) 1.0); // tone_n+1 =
                                                                                                                                               // tone_n*2^(1/12)
  // Decay window
  ...
  float timestampUnit = (float) 1.0e-6; // See a previous remark: could be detected before to avoid problem if we are working with a different scale of
                                        // timestamp...
  ...
  synchronized public EventPacket filterPacket(EventPacket in) {
    ...
    for (Object e : in) {
      ...
      if (i.polarity == PolarityEvent.Polarity.Off) {
        ...
        if (pixelsMap[x][y].otherPolarityOccurred == true) {
          ...
          if (((dt * pixelsMap[x][y].pixelTimestampUnit) > (1.0 / pixelsMap[x][y].frequencyHighBoundary))
            ...
            // System.out.println(String.format("measuredFrequencies: %.3f Hz", pixelsMap[x][y].measuredFrequencies.get_lastInsertedValue())); // DEBUG : SEEMS
            // TO BE WORKING
      ...
      // System.out.println(String.format("lowFineFreq = %.3f Hz\nhighFineFreq = %.3f Hz", lowFineFilterFreq, highFineFilterFreq)); // DEBUG : SEEMS TO BE
      // WORKING
  ...
  public void annotate(GLAutoDrawable drawable) {
      ...
      gl.glBegin(GL.GL_POINTS);
      {
        for (int cnt = 0; cnt < this.timestampsOfDisplayedPixels.bufferSize; cnt++) {
          ...
          if (pixelsMap[xCoordinate][yCoordinate].get_displayActive() == true) { // Yes, for the moment, it is a useless test...
            ...
            gl.glColor3f(pixelsMap[xCoordinate][yCoordinate].pixelRGB[0], pixelsMap[xCoordinate][yCoordinate].pixelRGB[1],
                pixelsMap[xCoordinate][yCoordinate].pixelRGB[2]);
            // We display the pixel
            gl.glVertex2i(xCoordinate, yCoordinate);
            this.pixelsMap[xCoordinate][yCoordinate].set_displayActive(false); // reset of the activeDisplay tag => indeed, for the moment, this tag is quite
                                                                               // useless
          }
        }
      }
    ...
    for (int cnt = 0; cnt < 12; cnt++) {
      ...
      glutColorCode.glutBitmapString(GLUT.BITMAP_HELVETICA_18, pixelsMap[0][0].octave.tones[cnt].name); // for getting the name, we pray that the Tone object
                                                                                                        // was initialized for the pixel (0, 0)...
    }
    ...
    glutColorCode.glutBitmapString(GLUT.BITMAP_HELVETICA_18, String.format("f_tone = %.3f Hz (%s%d)", presumedOctave.tones[presumedToneCnt].frequency,
        presumedOctave.tones[presumedToneCnt].name, presumedOctave.tones[presumedToneCnt].index));
  }
  ...
  public class Pixel {
    ...
    private float SaturationCorrection; // tuning of the saturation value (in HSL space) that depends on the correctness of the average frequency with respect
                                        // to the corresponding tone frequency.
    ...
    public Pixel(int bufferSize, float timestampUnitValue, float lowFrequencyBoundaryValue, float highFrequencyBoundaryValue) {
      ...
      this.octave = new Octave((int) Math.ceil(Math.log(Math.sqrt(this.frequencyLowBoundary * this.frequencyHighBoundary) / 27.5))); // beware, for the moment
                                                                                                                                     // it might be out of the
                                                                                                                                     // octaveRange
                                                                                                                                     // boundaries...
