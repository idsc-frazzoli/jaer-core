...
public class NotchFilter extends EventFilter2D implements Observer {
  ...
  void checkMap() {
    if (lastTimesMap == null || lastTimesMap.length != chip.getSizeX() + 2 || lastTimesMap[0].length != chip.getSizeY() + 2
        || lastTimesMap[0][0].length != chip.getNumCellTypes() || lastTimesMap[0][0][0].length != NUMTIMES) {
      allocateMap();
    }
  }
