...
public class ApsNoiseStatistics extends EventFilter2DMouseAdaptor implements FrameAnnotater, Observer, PropertyChangeListener {
  ...
  final private static float[] TEMPORAL_HIST_COLOR = { 0, 0, .8f, .3f }, SPATIAL_HIST_COLOR = { .6f, .4f, .2f, .6f },
  ...
  synchronized public EventPacket<?> filterPacket(EventPacket<?> in) {
    ...
    if (frameExtractor.hasNewFrame()) {
      if (resetCalled) {
        // frameExtractor.resetFilter(); // TODO we cannot call this before getting frame, because then frame will be set to zero in the frameExtractor and
        // we'll get zeros
        stats.reset();
        resetCalled = false;
      }
  ...
  public void propertyChange(PropertyChangeEvent evt) {
    if (isFilterEnabled() && resetOnBiasChange && evt.getSource() instanceof AEChip && evt.getPropertyName() != DavisChip.PROPERTY_FRAME_RATE_HZ
  ...
  class PixelStatistics {
    ...
    private class TemporalNoise {
      ...
      synchronized void draw(GL2 gl) {
        ...
        String s = String.format("Temporal noise: %.1f+/-%.2f var=%.1f COV=%.1f%% var/mean=k=%.2f DN/e, k=%s uV/e N=%d", meanmean, rmsAC, meanvar,
            (100 * rmsAC) / meanmean, kdn, engFmt.format(keuV), nPixels);
