...
public class RefractoryFilter extends EventFilter2D implements Observer, PropertyChangeListener {
  ...
  public RefractoryFilter(AEChip chip) {
    ...
    setPropertyTooltip("passShortISIsEnabled",
        "<html>Inverts filtering so that only events with short ISIs are passed through.<br>If refractoryPeriodUs==0, then you can block all events with idential timestamp from the same pixel.");
  }
  ...
  synchronized public EventPacket filterPacket(EventPacket in) {
    ...
    for (Object e : in) {
      ...
      int deltat = (ts - lastt);
      boolean longISI = lastt == DEFAULT_TIMESTAMP || deltat > refractoryPeriodUs; // if refractoryPeriodUs==0, then all events with ISI==0 pass if
