...
public class ApsDvsEventFilter extends EventFilter2D {
  ...
  public ApsDvsEventFilter(AEChip chip) {
    super(chip);
    setPropertyTooltip("filterFrameTransientEventsTimeUs",
        "Events within this time in us after end-of-frame are filtered out; used for filtering DVS events possibly tirggered by APS global shutter");
  ...
  public EventPacket<?> filterPacket(EventPacket<?> in) {
    ...
    while (fullIterator.hasNext()) {
      ...
      if (filterFrameTransientEvents && lastFrameStartTimestamp != Integer.MIN_VALUE
          && event.getTimestamp() - lastFrameStartTimestamp < filterFrameTransientEventsTimeUs) {
        continue;
      }
