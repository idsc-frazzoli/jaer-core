...
public class SpatioTemporalCorrelationFilter extends EventFilter2D implements Observer, FrameAnnotater {
  ...
  public SpatioTemporalCorrelationFilter(AEChip chip) {
    ...
    setPropertyTooltip(filt, "numMustBeCorrelated",
        "At least this number of 9 (3x3) neighbors (including our own event location) must have had event within past dt");
    setPropertyTooltip(adap, "activityBinDimBits",
        "2^this is the size of rectangular blocks that histogram event activity for measuring entropy (structure) to evaluate effectiveness of filtering");
  ...
  public void annotate(GLAutoDrawable drawable) {
    ...
    if (adaptiveFilteringEnabled) {
      s = String.format("dt=%.1fms, filteredOutPercent=%%%.1f, entropyReduction=%.1f", dt * 1e-3f, filteredOutPercent, entropyReduction);
    } else {
      s = String.format("filteredOutPercent=%%%.1f", filteredOutPercent);
    }
