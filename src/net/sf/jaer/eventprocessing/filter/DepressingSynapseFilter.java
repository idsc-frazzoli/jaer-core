...
public class DepressingSynapseFilter extends EventFilter2D implements FrameAnnotater {
  ...
  public DepressingSynapseFilter(AEChip chip) {
    ...
    setPropertyTooltip("tauMs",
        "recorery time constant in ms of depressing synapse; recovers full strength with this 1st order time constant; increase to make filtering last longer");
    setPropertyTooltip("weight",
        "weight of each incoming spike on depressing synapse; incrrease to reduce number of spikes within tauMs window to filter out events.");
  ...
  static class Neurons implements Serializable {
    ...
    private class Neuron implements Serializable {
      ...
      private float state = 0; // internal state, clips to 0:1 and the larger the value, the more depressed is the synapse. The probability to transmit is
                               // linear in (1-state).
