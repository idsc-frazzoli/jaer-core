...
public class CellStatsProber extends EventFilter2D implements FrameAnnotater, MouseListener, MouseMotionListener, Observer, PropertyChangeListener {
  ...
  public CellStatsProber(AEChip chip) {
    ...
    setPropertyTooltip(h, "showAverageISIHistogram",
    ...
    setPropertyTooltip(h, "separateEventTypes",
    ...
    setPropertyTooltip(l, "showLatencyHistogramToExternalInputEvents",
    ...
    // yet
    // implemented
    setPropertyTooltip(l, "externalInputEventAddress", "int32 address of external input events; see e.g. DavisChip.EXTERNAL_INPUT_EVENT_ADDR for this address");
    setPropertyTooltip(c, "countDVSEventsBetweenExternalPinEvents",
        "counts events of ON and OFF polarity between rising and falling phases of stimulus based on special events from external input pin");
  ...
  private class Stats {
    ...
    private class ISIHist {
      ...
      public String toString() {
        return String.format("ISIHist: virgin=%s maxCount=%d lessCount=%d moreCount=%d isiMaxUs=% isiMinUs=%d", virgin, maxCount, lessCount, moreCount,
            isiMaxUs, isiMinUs);
      }
    ...
    @Override
    public String toString() {
      return String.format("Selected region %6d pixels, total %10d events, %15s eps total, %15s eps/pixel", nPixels, count,
          engFmt.format(filteredRatePerPixel * nPixels), engFmt.format(filteredRatePerPixel));
    }
    ...
    private class EventCountsAfterInputPinEvents {
      ...
      public String toString() {
        ...
        return String.format(
            ...
            phase.toString(), numRisingEdges, numFallingEdges, onRisingCount, offRisingCount, engFmt.format(onEventsPerRisingPhasePerPixel),
            engFmt.format(offEventsPerRisingPhasePerPixel), onFallingCount, offFallingCount, engFmt.format(onEventsPerFallingPhasePerPixel),
            engFmt.format(offEventsPerFallingPhasePerPixel));
      }
  ...
  public void propertyChange(PropertyChangeEvent evt) {
    if (isFilterEnabled() && resetOnBiasChange && (evt.getSource() instanceof AEChip) && !evt.getPropertyName().equals(DavisChip.PROPERTY_FRAME_RATE_HZ)
        && !evt.getPropertyName().equals(DavisVideoContrastController.AGC_VALUES) && !evt.getPropertyName().equals(DavisChip.PROPERTY_MEASURED_EXPOSURE_MS)) {
      resetFilter();
