...
public class PcaTrackingFilter extends EventFilter2D implements FrameAnnotater {
  ...
  double R4 = 0f;
  /////////////////////////////////////////
  Point2D medianPoint = new Point2D.Float(), stdPoint = new Point2D.Float(), meanPoint = new Point2D.Float(), eigvec1Point = new Point2D.Float(),
      eigvec2Point = new Point2D.Float();
  ...
  public EventPacket filterPacket(EventPacket in) {
    ...
    for (Object o : in) {
      ...
      square1Point.setLocation(xmean + scale_stddev * stddev_inc * ((eigvec1x * stddev1) - (eigvec2x * stddev2)),
          ymean + scale_stddev * stddev_inc * ((eigvec1y * stddev1) - (eigvec2y * stddev2)));
      square2Point.setLocation(xmean + scale_stddev * stddev_inc * (-1 * (eigvec1x * stddev1) - (eigvec2x * stddev2)),
          ymean + scale_stddev * stddev_inc * (-1 * (eigvec1y * stddev1) - (eigvec2y * stddev2)));
      square3Point.setLocation(xmean + scale_stddev * stddev_inc * (-1 * (eigvec1x * stddev1) + (eigvec2x * stddev2)),
          ymean + scale_stddev * stddev_inc * (-1 * (eigvec1y * stddev1) + (eigvec2y * stddev2)));
      square4Point.setLocation(xmean + scale_stddev * stddev_inc * ((eigvec1x * stddev1) + (eigvec2x * stddev2)),
          ymean + scale_stddev * stddev_inc * ((eigvec1y * stddev1) + (eigvec2y * stddev2)));
      ...
      if ((java.lang.Math.abs(dot_eigvec1) <= (scale_stddev * stddev_inc * stddev1))
          && (java.lang.Math.abs(dot_eigvec2) <= (scale_stddev * stddev_inc * stddev2))) {
