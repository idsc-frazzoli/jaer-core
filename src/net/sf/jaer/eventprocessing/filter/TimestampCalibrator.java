...
public class TimestampCalibrator extends EventFilter2D implements FrameAnnotater, PropertyChangeListener {
  ...
  public TimestampCalibrator(AEChip chip) {
    ...
    setPropertyTooltip("ppmTimestampTooFastError",
        "<html>the calibration factor: <br>parts per million that the timestamp clock is too fast relative to computer clock. <br> I.e. +1000 means timestamps advance 0.1% too fast; <br>after 1000 ticks of computer clock, timestamp will be 1 tick ahead");
  ...
  synchronized public EventPacket<?> filterPacket(EventPacket<?> in) {
    if (in.isEmpty() || (enableCalibration && !(chip.getAeViewer().getPlayMode() == PlayMode.LIVE)) // go ahead and run rest if we are just correcting
                                                                                                    // timestamps
    ) {
      return in;
    }
  ...
  private class TimestampCalibration {
    ...
    public String toString() {
      return "TimestampCalibration{" + "ppmTimestampTooFastError=" + ppmTimestampTooFastError + ", vid=" + vid + ", pid=" + pid + ", did=" + did + ", date="
          + date + '}';
    }
  }
  ...
  public void doLoadCalibration() {
    ...
    try (Reader reader = new FileReader(fn)) {
      ...
      if (!(chip.getAeViewer().getPlayMode() == PlayMode.LIVE) || chip.getHardwareInterface() == null) {
        JOptionPane.showMessageDialog(chip.getAeViewer().getFilterFrame(),
            "Device not running live or no hardware interface, cannot check if calibration is valid for this device");
        return;
      }
      setPpmTimestampTooFastError(tsc.ppmTimestampTooFastError);
      USBInterface usbInterface = (USBInterface) chip.getHardwareInterface();
      if (usbInterface.getVID() != tsc.vid || usbInterface.getPID() != tsc.pid || usbInterface.getDID() != tsc.did) {
