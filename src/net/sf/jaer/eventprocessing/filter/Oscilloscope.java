...
public class Oscilloscope extends EventFilter2D implements Observer, FrameAnnotater, PropertyChangeListener {
  ...
  public Oscilloscope(AEChip chip) {
    ...
    setPropertyTooltip(t, "triggerType",
        "<html><ul>" + "<li>Manual: Triggers on button press" + "<li>TimeInterval: on time interval triggerIntervalUs"
            + "<li>EventInterval: on every triggerTimeIntervalUs" + "<li>SpecialEvent: on every special event"
            + "<li>BiasChange: on every property change from the AEChip's hardware configuration" + "</ul>");
    setPropertyTooltip(t, "triggerMode", "<html><ul>" + "<li>Auto: Triggers again after playbackNumberOfCycles plays of last capture are played"
        + "<li>Normal: Triggers again only if trigger condition is not met, after replays of current capture have been played" + "</ul>");
    ...
    setPropertyTooltip(c, "captureType", "<html><ul>" + "<li>TimeInterval: capture time interval lengthOfTimeToCaptureUs in us"
        + "<li>EventInterval: capture numberOfEventsToCapture events" + "</ul>");
    ...
    setPropertyTooltip(p, "playbackType", "<html><ul>" + "<li>EventNumber: play playbackNumEvents per rendered frame"
        + "<li>TimeInterval: play playbackTimeIntervalUs in us per rendered frame" + "</ul>");
