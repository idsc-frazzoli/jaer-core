...
public class SunriseSunset {
  ...
  private void doCalculations() {
    try {
      ...
      for (iCount = 0; iCount <= 1; iCount++) // Loop thru only twice
      {
        ...
        dfWW = -0.00010 - 0.04129 * Math.sin(dfLL * 2.0) + 0.03211 * Math.sin(dfGG) - 0.00104 * Math.sin(2.0 * dfLL - dfGG)
            - 0.00035 * Math.sin(2.0 * dfLL + dfGG) - 0.00008 * Math.sin(dfGG) * dfTT;
      ...
      if (bSunriseToday) {
        dateSunrise = dfmtDateTime.parse(iDay + " " + iMonth + " " + iYear + " " + (int) dfHourRise + ":" + (int) dfMinRise + " GMT");
      }
      // Load dateSunset with data
      if (bSunsetToday) {
        dateSunset = dfmtDateTime.parse(iDay + " " + iMonth + " " + iYear + " " + (int) dfHourSet + ":" + (int) dfMinSet + " GMT");
      }
    } // end of try
    // Catch errors
  ...
  public boolean isDaytime() {
    ...
    if (bSunriseToday && bSunsetToday) // sunrise and sunset
    {
      ...
      {
        if ((dateInput.after(dateSunrise) || dateInput.equals(dateSunrise)) || // use OR rather than AND
