...
public class RectangularClusterTracker extends EventFilter2D
  ...
  public RectangularClusterTracker(AEChip chip) {
    ...
    final String sizing = "Sizing", mov = "Movement", life = "Lifetime", disp = "Display", global = TOOLTIP_GROUP_GLOBAL, update = "Update", logg = "Logging",
        pi = "PI Controller";
  ...
  protected class ClusterLogger {
     ...
     * the event packet.
     * @param logNumber
     * the case # (analagous to frame) of this logg record. */
      ...
      if (logStream != null) {
        if (parseablePrint) {
          ...
          for (Cluster c : clusters) {
            ...
            logStream.print(String.format("%d %d %d %e %e ", c.lastEventTimestamp, (int) c.location.x, (int) c.location.y, c.velocityPPS.x, c.velocityPPS.y));
            printedClusters++;
    ...
    do {
      ...
      outer: for (int i = 0; i < nc; i++) {
        ...
        for (int j = i + 1; j < nc; j++) {
          ...
          if (overlapping && (velAngDiffDegToNotMerge > 0) && c1.isVisible() && c2.isVisible() && c1.isVelocityValid() && c2.isVelocityValid()
              && (c1.velocityAngleToRad(c2) > ((velAngDiffDegToNotMerge * Math.PI) / 180))) {
   ...
   *
   * @param t
   * the timestamp of the purge operation */
   ...
   *
   * @param t
   * the global timestamp of the update. */
   ...
   * the spawning event.
   * @param itr
   * the output iterator to write events to when they fall in this
   * cluster.
   * @return a new empty Cluster */
  ...
  public class Cluster implements ClusterInterface { // TODO badly needs a cheap contains(ev) method that accounts for
     ...
     *
     * @param ev
     * the event to center the cluster on.
     * @param outItr
     * used to generate the new event pointing back to the
     * cluster. */
     ...
     * the event.
     * @param outItr
     * the output iterator; used to generate new output event
     * pointing to cluster. */
     ...
     *
     * @param from
     * the first instantaneousAngle
     * @param to
     * the second instantaneousAngle
    ...
    protected float distanceToX(BasicEvent event) {
      ...
      float distance = Math
          .abs((((event.x - location.x) + (velocityPPT.x * (dt))) * cosAngle) + (((event.y - location.y) + (velocityPPT.y * (dt))) * sinAngle));
    ...
    protected float distanceToY(BasicEvent event) {
      int dt = event.timestamp - lastUpdateTime;
      float distance = Math
          .abs((((event.y - location.y) + (velocityPPT.y * (dt))) * cosAngle) - (((event.x - location.x) + (velocityPPT.x * (dt))) * sinAngle));
    ...
    public boolean checkAndSetClusterVisibilityFlag(int t) {
      ...
      if (useVelocity) {
        double speed = (Math.sqrt((velocityPPT.x * velocityPPT.x) + (velocityPPT.y * velocityPPT.y)) * 1e6) / AEConstants.TICK_DEFAULT_US; // speed is in
                                                                                                                                           // pixels/sec
    ...
    public void updatePath(int t) {
      ...
      if (numEvents == previousNumEvents) {
        return; // don't add point unless we had events that caused change in path (aside from prediction from
        // velocityPPT)
      }
    ...
    public String toString() {
      return String.format(
          ...
          getClusterNumber(), numEvents, (int) location.x, (int) location.y, lastUpdateTime, radiusX, radiusY, angle, getMass(), getLifetime(), isVisible(),
          getSpeedPPS());
    }
    ...
    private class RollingVelocityFitter {
      ...
      @Override
      public String toString() {
        return String.format("RollingVelocityFitter: \n" + "valid=%s nPoints=%d\n" + "xVel=%f, yVel=%f\n" + "st=%f sx=%f sy=%f, sxt=%f syt=%f den=%f", valid,
            nPoints, xVelocity, yVelocity, st, sx, sy, sxt, syt, den);
      }
   ...
   *
   * @param enableClusterExitPurging
   * the enableClusterExitPurging to set */
  ...
  /** @param showClusterRadius
   * the showClusterRadius to set */
   ...
   *
   * @param surround
   * the updateShape factor, constrained >1 by setter. radius
   ...
   *
   * @param pathsEnabled
   * true to show the history of the cluster locations on
   * each packet. */
   ...
   *
   * @param dynamicSizeEnabled
   * true to enable scaling of cluster size */
  ...
  /** @param filterEventsEnabled
   * the filterEventsEnabled to set */
   ...
   *
   * @param growMergedSizeEnabled
   * true to grow the cluster size, false to use
   ...
   *
   * @param showAllClusters
   * true to show all clusters even if there are not
   * "visible" */
   ...
   *
   * @param thresholdVelocityForVisibleCluster
   ...
   *
   * @param pathLength
   * the number of recorded path points. If <2, set to 2. */
  ...
  /** @param velAngDiffDegToNotMerge
   * the velAngDiffDegToNotMerge to set */
  ...
  /** @param showClusterEps
   * the showClusterEps to set */
  ...
  /** @param showClusterVelocity
   * the showClusterVelocity to set */
  ...
  /** @param velocityVectorScaling
   * the velocityVectorScaling to set */
  ...
  /** @param initializeVelocityToAverage
   * the initializeVelocityToAverage to set */
  ...
  /** @param showClusterMass
   * the showClusterMass to set */
  ...
  /** @param clusterLoggingMethod
   * the clusterLoggingMethod to set */
  ...
  /** @param surroundInhibitionEnabled
   * the surroundInhibitionEnabled to set */
  ...
  /** @param angleFollowsVelocity
   * the angleFollowsVelocity to set */
  ...
  /** @param useEllipticalClusters
   * the useEllipticalClusters to set */
  ...
  /** @param defaultClusterRadius
   * the defaultClusterRadius to set */
