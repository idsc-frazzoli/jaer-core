...
import ch.unizh.ini.jaer.projects.util.WeightedEvent;
...
public class HoughCircleTracker extends EventFilter2D implements FrameAnnotater, Observer {
  ...
  public void annotate(GLAutoDrawable drawable) {
    ...
    if (drawHough == true) {
      for (int x = 0; x < cameraX; x++) {
        for (int y = 0; y < cameraY; y++) {
          ...
          gl.glRectf(x - 0.5f, y - 0.5f, x + 0.5f, y + 0.5f);
        }
      }
    }
    // draw the circle
    ...
    for (int j = 0; j < nrMax; j++) {
      ...
      gl.glRectf(maxCoordinate[j].x - 1.0f, maxCoordinate[j].y - 1.0f, maxCoordinate[j].x + 1.0f, maxCoordinate[j].y + 1.0f);
  ...
  boolean islocmax(int x, int y) {
    int locMaxRad = 1;
    int i = 1;
    if (((x - locMaxRad) < 0) || ((x + locMaxRad) > (chip.getSizeX() - 1)) || ((y - locMaxRad) < 0) || ((y + locMaxRad) > (chip.getSizeY() - 1))) {
      return false;
    }
  ...
  synchronized public EventPacket<?> filterPacket(EventPacket<?> in) {
    ...
    for (int x = 0; x < cameraX; x++) {
      for (int y = 0; y < cameraY; y++) {
        for (int i = 0; i < nrMax; i++) {
          if ((accumulatorArray[x][y] >= maxValue[i]) && islocmax(x, y)) {
            maxValue[i] = accumulatorArray[x][y];
    ...
    for (int i = 0; i < nrMax; i++) {
      ...
      if (locDepression == true) {
        if (((x - 1) > 0) && ((x + 1) < (chip.getSizeX() - 1)) && ((y - 1) > 0) && ((y + 1) < (chip.getSizeY() - 1))) {
