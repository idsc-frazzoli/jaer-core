...
public class ParticleTracker extends EventFilter2D implements FrameAnnotater, Observer {
  ...
  public ParticleTracker(AEChip chip) {
    ...
    setPropertyTooltip("clusterUnsupportedLifetime",
        "prune clusters that don't get events for this long in ticks; also time constant of effect of event - decrease to move clusters more rapidly to new events");
          ...
          for (i = 0; i < old_ids.length; i++) {
            if (old_ids[i] == old_new_id.o) {
              ...
              if (logStream != null) {
                ...
                logStream.println(String.format("%% %d <- %d", old_new_id.n, min_old_id)); // trying to make particles trackable from the log, logging their ID
                                                                                           // and logging changes of ID tags
              }
