...
public class EinsteinClusterTracker extends EventFilter2D implements FrameAnnotater, Observer /* , PreferenceChangeListener */ {
  ...
  // protected float velocityMixingFactor=getPrefs().getFloat("RectangularClusterTracker.velocityMixingFactor",0.0005f); // mixing factor for velocityPPT
  // computation
  ...
  private final float VELOCITY_VECTOR_SCALING = 1e5f; // to scale rendering of cluster velocityPPT vector, velocityPPT is in pixels/tick=pixels/us so this gives
                                                      // 1 screen pixel per 10 pix/s actual vel
  private boolean useVelocity = getPrefs().getBoolean("RectangularClusterTracker.useVelocity", true); // enabling this enables both computation and rendering of
                                                                                                      // cluster velocities
  ...
  private boolean useNearestCluster = getPrefs().getBoolean("RectangularClusterTracker.useNearestCluster", false); // use the nearest cluster to an event, not
                                                                                                                   // the first containing it
  ...
  public EinsteinClusterTracker(AEChip chip) {
    ...
    setPropertyTooltip(movement, "useVelocity", "uses measured cluster velocity to predict future position; vectors are scaled "
    ...
    setPropertyTooltip(lifetime, "clusterLifetimeIncreasesWithAge",
        "older clusters can live longer (up to clusterLifetimeWithoutSupportUs) without support, good for objects that stop (like walking flies)");
  ...
  public class Cluster {
    ...
    public String toString() {
      return String.format("Cluster number=#%d numEvents=%d locationX=%d locationY=%d radiusX=%.1f radiusY=%.1f lifetime=%d visible=%s velocityPPS=%.2f",
          getClusterNumber(), numEvents, (int) location.x, (int) location.y, radiusX, radiusY, getLifetime(), isVisible(), getVelocityPPS());
    }
    ...
    private class RollingVelocityFitter {
      ...
      private synchronized void update() {
        ...
        sxt += p.x * t;
        syt += p.y * t;
        // if(n<length) return; // don't estimate velocityPPT until we have all necessary points, results very noisy and send cluster off to infinity very
        // often, would give NaN
