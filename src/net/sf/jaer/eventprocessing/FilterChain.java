...
public class FilterChain extends LinkedList<EventFilter2D> {
  ...
  public FilterChain(AEChip chip) {
    ...
    timedOut = false;
    try {
      processingMode = ProcessingMode.valueOf(chip.getPrefs().get("FilterChain.processingMode", FilterChain.ProcessingMode.RENDERING.toString())); // ProcessingMode.RENDERING;
    } catch (Exception e) {
      e.printStackTrace();
    }
  }
    ...
    try {
      ...
      for (String s : classNames) {
        try {
          ...
          log.warning(
              "couldn't construct filter " + s + " for chip " + chip.getClass().getName() + " : " + e.toString() + " will remove this filter from Preferences");
          toRemove.add(s);
          if (e.getCause() != null) {
          ...
          log.warning("couldn't construct filter " + s + " for chip " + chip.getClass().getName() + " : " + err.toString()
              + " will remove this filter from Preferences");
          toRemove.add(s);
  ...
  synchronized void customize() {
    ...
    if (chooser.getReturnStatus() == ClassChooserDialog.RET_OK) {
      ...
      List<String> newClassNames = chooser.getList();
      try {
  ...
  private void storePreferredFilterPreferences(List<String> newClassNames) throws IOException, BackingStoreException {
