...
abstract public class AbstractDirectionSelectiveFilter extends EventFilter2D implements Observer, FrameAnnotater {
  ...
  protected void checkMap() {
    if ((lastTimesMap == null) || (lastTimesMap.length != (chip.getSizeX() + (2 * getSearchDistance())))
        || (lastTimesMap[0].length != (chip.getSizeY() + (2 * getSearchDistance()))) || (lastTimesMap[0][0].length != NUM_INPUT_TYPES)) {
      allocateMap();
    }
  }
  ...
  public void annotate(GLAutoDrawable drawable) {
    ...
    if (isShowGlobalEnabled()) {
      ...
      gl.glPopMatrix();
      // </editor-fold>
      // <editor-fold defaultstate="collapsed" desc="-- draw global expansion as circle with radius proportional to expansion metric, smaller for contraction,
      // larger for expansion --">
      gl.glPushMatrix();
