...
public class SimpleOrientationFilter extends AbstractOrientationFilter {
  ...
  synchronized public EventPacket<?> filterPacket(EventPacket<?> in) {
    if (enclosedFilter != null)
      in = enclosedFilter.filterPacket(in); // TODO filters out the IMU samples sometimes because the iterator is not the fullIterator
    if (in.getSize() == 0)
      return in;
    ...
    for (Object ein : in) {
        ...
        for (int ori = 0; ori < NUM_TYPES; ori++) {
          ...
          if (count > 0) {
            ...
            if (oriDecideHelper[ori] < 0)
              oriDecideHelper[ori] = Integer.MAX_VALUE; // Happens when dtList[k]^2 is larger than maxint or the sum exceeds maxint.
      ...
      if (!multiOriOutputEnabled) {
        ...
        if (dir == -1) { // didn't find a good orientation
          if (passAllEvents)
            writeOutput(outItr, e, false, (byte) 0);
          continue;
        }
