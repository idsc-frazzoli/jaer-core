...
abstract public class AbstractOrientationFilter extends EventFilter2D implements Observer, FrameAnnotater {
  ...
  protected final Dir[] baseOffsets = { new Dir(1, 0), // right
  ...
  public AbstractOrientationFilter(AEChip chip) {
    ...
    setPropertyTooltip(hist, "oriHistoryDiffThreshold",
        "detected orientation must be within this value of historical value to pass. Value of 0.5 corresponds to 45degree with 4 directions.");
  }
  ...
  protected void checkMaps(EventPacket packet) {
    if ((lastTimesMap == null) || (lastTimesMap.length != chip.getSizeX()) || (lastTimesMap[0].length != chip.getSizeY()) || (lastTimesMap[0][0].length != 2)) { // changed
                                                                                                                                                                 // to
                                                                                                                                                                 // 2
                                                                                                                                                                 // for
                                                                                                                                                                 // PolarityEvents
  ...
  synchronized protected void allocateMaps() {
    ...
    if (chip != null) {
      ...
      log.info(String.format("allocated int[%d][%d][%d] array for last event times and float[%d][%d] array for orientation history", chip.getSizeX(),
          chip.getSizeY(), 2, chip.getSizeX(), chip.getSizeY()));
    }
    computeRFOffsets();
  }
