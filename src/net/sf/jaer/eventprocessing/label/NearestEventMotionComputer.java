 ...
 * the non-oriented events and is only useful for small particles where nearest neighbor makes sense. Even then for particles that are larger than one pixel it
 * will
 * make lots of errors.
 *
 * @author tobi */
...
public class NearestEventMotionComputer extends EventFilter2D implements Observer, FrameAnnotater {
  ...
  void checkMap(EventPacket in) {
    if ((lastTimesMap == null) || (lastTimesMap.length != (chip.getSizeX() + PADDING)) || (lastTimesMap[0].length != (chip.getSizeY() + PADDING))
        || (lastTimesMap[0][0].length != in.getNumCellTypes())) {
  ...
  final Dir[] offsets = { new Dir(0, -1), // down
