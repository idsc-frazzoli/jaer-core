...
public abstract class EventFilter extends Observable implements HasPropertyTooltips {
  ...
  public void setEnclosedFilterChain(FilterChain enclosedFilterChain) {
    ...
    if (enclosedFilterChain.isEmpty()) {
      log.warning("empty filter chain in " + this
          + " - you should set the filter chain after all filters have been added to it so that enclosed filters can be processed");
    }
   ...
   * parameters:
   * <pre>
   * select-word copy-to-clipboard caret-begin-line caret-down "{setPropertyTooltip(\"" paste-from-clipboard "\",\"\");}" insert-break caret-up caret-end-line
   * caret-backward caret-backward caret-backward caret-backward
   * </pre>
   *
