...
public class AEUnicastInput implements AEUnicastSettings, PropertyChangeListener {
  ...
  private void allocateBufffers() {
    ...
    for (int i = 0; i < NBUFFERS; i++) {
      ...
      buffer.order(swapBytesEnabled || spinnakerProtocolEnabled || secDvsProtocolEnabled ? ByteOrder.LITTLE_ENDIAN : ByteOrder.BIG_ENDIAN); // spinnaker always
                                                                                                                                            // uses little
                                                                                                                                            // endian
  ...
  public AENetworkRawPacket readPacket() {
    ...
    try {
      returnearly: while (filledBufferQueue.peek() != null) {
  ...
  private void checkSequenceNumber(ByteBuffer buffer) {
    if (sequenceNumberEnabled) {
      ...
      if (datagramSequenceNumber != datagramCounter) {
        log.warning(
            String.format("Dropped %d packets. (Incoming packet sequence number (%d) doesn't match expected packetCounter (%d), resetting packetCounter)",
                datagramSequenceNumber - datagramCounter, datagramSequenceNumber, datagramCounter));
  ...
  private static ByteBuffer clone(ByteBuffer original) {
    ...
    if (clone.capacity() < 90000) { // The max length of a whole frame packet is 86464, so the capacity must be bigger than 86464, here we use 90000 for
                                    // convience.
  ...
  private void extractEvents(ByteBuffer buffer, AENetworkRawPacket packet) {
    if (isSpinnakerProtocolEnabled()) {
      ...
      buffer.get();
      // read events
      int ts = !timestampsEnabled || localTimestampsEnabled ? (int) (((System.nanoTime() / 1000) << 32) >> 32) : 0; // if no timestamps coming, add system clock
                                                                                                                    // for all.
        ...
        if (use4ByteAddrTs) {
          ...
          if (timestampsEnabled && !localTimestampsEnabled) {
            ...
            if ((floatFinalTime >= Integer.MAX_VALUE) || (floatFinalTime <= Integer.MIN_VALUE)) {
              ...
              finalTime = Integer.MIN_VALUE + (int) (floatFinalTime - Integer.MAX_VALUE); // Change to -2k seconds now - was: wrap around at 2k seconds, back to
                                                                                          // 0 seconds. TODO different than hardware which wraps back to -2k
                                                                                          // seconds
            } else {
      ...
      // deprecated ... int ts = !timestampsEnabled || localTimestampsEnabled ? (int)( System.nanoTime() / 1000 ) : 0; // if no timestamps coming, add system
      // clock for all.
      int ts = !timestampsEnabled || localTimestampsEnabled ? (int) (((System.nanoTime() / 1000) << 32) >> 32) : 0; // if no timestamps coming, add system clock
                                                                                                                    // for all.
      ...
      if (cAERStreamEnabled) {
        try {
          ...
          if (nEventsNum != 0) { // This is a valid packet's head buffer
            ...
            } catch (BufferOverflowException e) { // Sometimes the buffer's order may be wrong which will result in the bufferoverflow, so just reset the
                                                  // wholePktBuffer in this case
              wholePktBuffer = null;
              return;
            }
            if (wholePktBuffer.position() == (jaer3PktSize * jaer3PktNum + 28)) { // This is the end buffer, the packet is finished.
        ...
        if (addressFirstEnabled) {
          if (use4ByteAddrTs) {
            ...
            if (timestampsEnabled && !localTimestampsEnabled) {
              ...
              if ((floatFinalTime >= Integer.MAX_VALUE) || (floatFinalTime <= Integer.MIN_VALUE)) {
                timeZero = rawTime; // after overflow reset timezero
                finalTime = Integer.MIN_VALUE + (int) (floatFinalTime - Integer.MAX_VALUE); // Change to -2k seconds now - was: wrap around at 2k seconds, back
                                                                                            // to 0 seconds. TODO different than hardware which wraps back to
                                                                                            // -2k seconds
              } else {
                finalTime = (int) floatFinalTime;
              }
            ...
            eventRaw.address = maybeSwapByteOrder(buffer.getShort()) & 0xffff; // swab(buffer.getShort()); // swapInt is switched to handle big endian event
                                                                               // sources (like ARC camera)
            // eventRaw.timestamp=(int) (timestampMultiplier*(int) swab(buffer.getShort()));
            if (!localTimestampsEnabled && timestampsEnabled) {
  ...
  private boolean checkSocket() {
    ...
    try {
      ...
      // log.warning("datagram socket read timeout value read=" + datagramSocket.getSoTimeout() + " which is different than timeout value of " + TIMEOUT_MS + "
      // that we tried to set - perhaps timeout is not supported?");
      // }
