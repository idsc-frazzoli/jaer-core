...
public class AEInputStream implements Closeable {
  ...
  public static final String EVENT_EOF = "eof", EVENT_WRAPPED_TIME = "wrappedTime", EVENT_POSITION = "position", EVENT_REWIND = "rewind",
      EVENT_MARKSET = "markset", EVENT_MARKS_CLEARED = "marksCleared", EVENT_MARK_IN_SET = "markInSet", EVENT_MARK_OUT_SET = "markOutSet", EVENT_INIT = "init",
      EVENT_NON_MONOTONIC_TIMESTAMP = "nonMonotonicTimestamp", EVENT_REPOSITIONED = "respositioned", EVENT_REPEAT_ON = "repeatOn",
  ...
  public AEPacketRaw readPacketByTime(int dt) throws IOException {
    packet.setNumEvents(0);
    int ts;
    if (!readFirstEventFlag) {
      lastPacketEndTimestamp = readEvent().timestamp;
    }
    do {
    ...
    while (((char) (c = pbis.read())) != '\r') {
      if ((c < 32) || (c > 126)) {
        throw new IOException(
            "Non printable character (<32 || >126) detected in header line, aborting header read and resetting to start of file because this file may not have a real header");
      }
      s.append((char) c);
    }
    if ((c = pbis.read()) != '\n') {
