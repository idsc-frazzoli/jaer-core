...
public class Jaer3BufferParser {
  ...
  public class PacketHeader {
    ...
    public String toString() {
      return "PacketHeader{" + "eventType=" + eventType + ", eventSource=" + eventSource + ", eventSize=" + eventSize + ", eventTSOffset=" + eventTSOffset
          + ", eventTSOverflow=" + eventTSOverflow + ", eventCapacity=" + eventCapacity + ", eventNumber=" + eventNumber + ", eventValid=" + eventValid + '}';
    }
  }
  ...
  private PacketDescriptor searchPacketHeader(int startPosition, int direction) throws IOException {
    ...
    while ((in.position() <= (in.limit() - PKT_HEADER_SIZE)) && (in.position() >= 0)) {
      ...
      d.eventTSOverflow = in.getInt(); // overflow counter, using to generate 64 bit timestamp for systems that have this (not jaer, it handles big wraps in
                                       // rendering)
      ...
      {
        if ((d.eventType == EventType.SpecialEvent) || (d.eventType == EventType.PolarityEvent) || (d.eventType == EventType.SampleEvent)
            || (d.eventType == EventType.EarEvent)) {
  ...
  public int getNextValidEventOffset() throws IOException {
    ...
    while (((eventFirstInt & validMask) != 1) || (currentPkt.pktHeader.eventType == EventType.Imu6Event)
        || (currentPkt.pktHeader.eventType == EventType.Imu9Event)) {
    ...
    if (currentPkt.pktHeader.eventType == EventType.FrameEvent) { // TODO why does it parse the frame header over and over again
      ...
      if (translatedArrayIndex >= 0) {
        ...
        if ((translatedArrayIndex >= (numPixels)) && (translatedArrayIndex <= ((2 * numPixels) - 1))) {
          jaer2FrameAddr = ((((translatedArrayIndex - (numPixels)) / ylength)) << 17) + (((translatedArrayIndex - (numPixels)) % ylength) << 2) + 0;
  ...
  public long bufferNumEvents() throws IOException {
    ...
    while (pkt != null) {
      ...
      if (pkt.pktHeader.eventType == EventType.FrameEvent) {
        ...
        framePktNum += 1;
        numEvents += 2 * (xlength * ylength * (pkt.pktHeader.eventValid)); // One array has been divided into 2 arrays, so the event numbers in Frame should
                                                                           // also increase
      }
      try {
        validPkttNum += 1;
