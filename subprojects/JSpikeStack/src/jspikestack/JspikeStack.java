...
public class JspikeStack {
  ...
  public static void retinaDemo() {
    ...
    List<PSPInput> events = AERFile.getRetinaEvents();
  ...
  public static void learningDemo() {
    ...
    List<PSPInput> events = AERFile.getCochleaEvents("VowelSounds.aedat");
    // Construct Network
    NetController<AxonSTP, AxonSTP.Globals, UnitLIF.Globals> nc = new NetController(NetController.AxonTypes.STP);
    Network<AxonSTP> net = nc.net;
  ...
  public static void convDemo() {
    ...
    un.tref = 5000;
    // Read Events
    List<PSPInput> events = AERFile.getRetinaEvents("MovingDarkBox.aedat");
    // Setup simulation
    nc.getControls().buildersEnabled = true;
    nc.addAllControls();
    nc.setRecordingState(true);
  ...
  public static void numberDemo() {
    ...
    // Read Events
    List<PSPInput> events = AERFile.getRetinaEvents(null);
  ...
  public static enum Demos {
    GENERATE, LEARN, CONV, RCNET, RETINA
  };
  ...
  public static void demoMenu() {
    ...
    addDemoButton("Convolution Demo", "Here we read data from the Silicon retina.  Two output layers respond to vertically and horizontally oriented features.",
        Demos.CONV, pane);
