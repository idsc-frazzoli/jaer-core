  ...
  public void eatEvents(int timeout) {
    ...
    while (!(inputBuffer.isEmpty() && (internalBuffer.isEmpty() || liveMode)) && enable) {
      ...
      try {
        ...
        // System.out.println("You tried firing an event at address with address "+ev.addr+" to Layer "+ev.layer+", which has just "+lay(ev.layer).nUnits()+"
        // units.");
        throw new java.lang.ArrayIndexOutOfBoundsException("You tried firing an event at address with address " + ev.sp.addr + " to Layer " + ev.sp.layer
            + ", which has just " + lay(ev.sp.layer).nUnits() + " units.");
      }
    }
