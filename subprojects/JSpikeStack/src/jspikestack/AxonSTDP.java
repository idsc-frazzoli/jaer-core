...
public class AxonSTDP<GlobalParams extends AxonSTDP.Globals> extends Axon<GlobalParams, PSPUnitToLayer> {
  ...
  // }
  //
  // public static class Layer <NetType extends STDPAxon,LayerType extends AxonBundle, UnitType extends Unit,GlobalParams extends STDPAxon.Layer.Globals>
  // extends AxonBundle<NetType,LayerType,UnitType>
  // {
  ...
  public void stdpLearn() {
    ...
    while (!postsyn.isEmpty() && (postsyn.peek().time + glob.stdpWin < net.time)) { // Iterate over new output spikes
      ...
      int i = 0;
      while (preit.hasNext()) { // Iterate over input events (from this layer) pertaining to the output event
