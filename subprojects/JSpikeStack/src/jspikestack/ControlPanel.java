 ...
 * the built in EventFilter method {@link net.sf.jaer.eventprocessing.EventFilter#addPropertyToGroup}. All properties of a given group are grouped together.
 * Within
        ...
        try {
          ...
          if (setter == null) {
            if (!printedSetterWarning) {
              log.warning(
                  "in filter " + getControllable() + " there is no setter for property change from property named " + propertyChangeEvent.getPropertyName());
